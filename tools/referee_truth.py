...
def send_result_block(player: Player, side: Side, st: GameState, m: MapData,
    ...
    # --- TRUTH dump (verification only; not in the distribution) ---
    import sys as _sys
    own_w = sorted([w for w in st.warriors.values() if w.hp > 0 and w.side.value == u], key=lambda w: wkey_sort(w.key))
    own_b = sorted([b for b in st.buildings.values() if b.side.value == u], key=lambda b: b.region)
    en_b_vis = sorted([b for b in st.buildings.values() if b.side.value != u and b.region in vis], key=lambda b: b.region)
    en_w_vis = sorted([w for w in st.warriors.values() if w.hp > 0 and w.side.value != u and w.region in vis], key=lambda w: wkey_sort(w.key))
    en_alive = sum(1 for w in st.warriors.values() if w.hp > 0 and w.side.value != u)
    en_hq = [b for b in st.buildings.values() if b.side.value != u and b.kind is BKind.HQ]
    _sys.stderr.write("TRUTH %s t=%d gold=%d vis=%d EA=%d EHQ=%d W=%s B=%s EB=%s EW=%s\n" % (
        side.name_str, st.day, st.gold[u], len(vis), en_alive, en_hq[0].level if en_hq else 0,
        ",".join("%s:%d:%d" % (id_str(w.key), w.region, w.hp) for w in own_w),
        ",".join("%d:%s:%d:%d" % (b.region, "HQ" if b.kind is BKind.HQ else "BASE", b.level, b.hp) for b in own_b),
        ",".join("%d:%s:%d:%d" % (b.region, "HQ" if b.kind is BKind.HQ else "BASE", b.level, b.hp) for b in en_b_vis),
        ",".join("%s:%d:%d" % (id_str(w.key), w.region, w.hp) for w in en_w_vis)))
