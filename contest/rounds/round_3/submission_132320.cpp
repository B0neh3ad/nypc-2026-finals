...
static int warrior_is_reserved(const Warrior *w) {
  return w->id.num == g_role_scout || g_claim_of[w->id.num] >= 0;
}
...
static int hq_defense(Actions *a, const GameState *S, const GameMap *M, const Paths *P, int *budget, int turn) {
  ...
  for (int i = 0; i < S->warriors.len; ++i) {
    const Warrior *w = &S->warriors.data[i];
    if (w->id.side == M->my_side) continue;
    if (g_hop_my[w->region] > 6) continue;
  ...
  for (int i = 0; i < S->warriors.len; ++i) {
    ...
    if (walk_len(P, w->region, M->my_hq) > 8) continue;
...
/* choose next stronghold to claim; -1 if none sensible */
static int choose_claim_target(const GameState *S, const GameMap *M, const Paths *P, int from, int aggressive) {
  int best = -1; long long best_score = 0;
  for (int i = 0; i < M->strongholds.len; ++i) {
    int r = M->strongholds.data[i];
    if (find_building_const(S, r) != NULL) continue;
    ...
    if (!aggressive && enemy_building_within(S, M, r, V5_CONTESTED_HOPS)) continue;
    int wl = walk_len(P, from, r);
    if (wl >= INF_HOPS) continue;
    long long score = (long long)wl * 10 + (enemy_side ? 200 : 0) + (g_hop_opp[r] <= 3 ? 500 : 0);
    if (best < 0 || score < best_score) { best = r; best_score = score; }
  }
  return best;
}

/* send claimers: spare HQ bodies -> nearest unclaimed strongholds */
static int issue_claims(Actions *a, const GameState *S, const GameMap *M, const Paths *P, int *budget,
                        int income_net, int *want_train, int turn) {
  int inflight = claims_in_flight(S, M);
  int issued = 0;
  int hq_keep = HQ_LEVELS[my_hq_level(S, M)].work_cap;  /* claimers may take the guard slot */
  while (inflight < V5_MAX_CLAIMS_IN_FLIGHT) {
    /* affordability: the build must be payable when the claimer arrives */
    if (*budget + income_net * 6 < BASE_LEVELS[1].cost * (inflight + 1)) break;
    int aggressive = (turn >= 150 && my_alive(S, M) >= enemy_alive_estimate(S));
    int target = choose_claim_target(S, M, P, M->my_hq, 0);
    if (target < 0 && aggressive) target = choose_claim_target(S, M, P, M->my_hq, 1);
    if (target < 0) break;
    /* source: a free body at any own building, nearest to target, that is surplus there */
    const Warrior *best = NULL; int best_len = INF_HOPS;
    for (int i = 0; i < S->warriors.len; ++i) {
      const Warrior *w = &S->warriors.data[i];
    ...
    ++inflight; ++issued;
    DBG("CLAIM %c%d -> %d (%d turns)\n", side_char(best->id.side), best->id.num, target, best_len);
  }
  return issued;
}
...
static void issue_upgrades(Actions *a, const GameState *S, const GameMap *M, int *budget, int turn, int reserve) {
  ...
  g_upgrade_reserve = 0;
#define WANT(cost) do { if (g_upgrade_reserve == 0) g_upgrade_reserve = (cost); } while (0)
  /* HQ2 early: hp5 warriors, turret 2, +1 slot */
  const Building *hq = find_building_const(S, M->my_hq);
  if (hq && hq->level == 1 && bases >= 2 && turn >= 25 && g_my_stat[M->my_hq] > 0 && !action_has_upgrade(a, M->my_hq)) {
    if (spend >= HQ_LEVELS[2].upgrade_cost) {
      VEC_PUSH(a->upgrades, M->my_hq); *budget -= HQ_LEVELS[2].upgrade_cost; spend -= HQ_LEVELS[2].upgrade_cost;
      DBG("UPGRADE HQ -> 2\n");
    } else WANT(HQ_LEVELS[2].upgrade_cost);
  }
  /* base L1->L2 (550 for +15/day) then L2->L3 (600) — only where the slot can be staffed soon */
  for (int lvl = 1; lvl <= 2; ++lvl) {
    for (int i = 0; i < S->buildings.len; ++i) {
      ...
      int cost = BASE_LEVELS[lvl + 1].cost;
      /* current slots full?  otherwise the upgrade adds nothing yet */
      if (g_my_stat[r] + g_my_arriving[r] < BASE_LEVELS[lvl].work_cap) continue;
      if (lvl == 2 && bases < 3) continue;
      if (spend < cost) { WANT(cost); continue; }
      VEC_PUSH(a->upgrades, r); *budget -= cost; spend -= cost;
      DBG("UPGRADE base %d -> %d\n", r, lvl + 1);
    }
  }
  /* HQ3..5 */
  hq = find_building_const(S, M->my_hq);
  if (hq && hq->level >= 2 && hq->level < HQ_MAX_LEVEL && g_my_stat[M->my_hq] > 0 && !action_has_upgrade(a, M->my_hq)) {
    int cost = HQ_LEVELS[hq->level + 1].upgrade_cost;
    int ok = 0;
    if (hq->level == 2 && (bases >= 4 || turn >= 120)) ok = 1;
    if (hq->level == 3 && (bases >= 6 || turn >= 170)) ok = 1;
    if (hq->level == 4 && (bases >= 7 || turn >= 240)) ok = 1;
    if (ok && spend >= cost) { VEC_PUSH(a->upgrades, M->my_hq); *budget -= cost; spend -= cost; DBG("UPGRADE HQ -> %d\n", hq->level + 1); }
    else if (ok) WANT(cost);
  }
#undef WANT
  /* max-level repairs when damaged and rich; endgame tiebreak */
  hq = find_building_const(S, M->my_hq);
  if (hq && hq->level == HQ_MAX_LEVEL && hq->hp < HQ_LEVELS[HQ_MAX_LEVEL].hp && !action_has_upgrade(a, M->my_hq) &&
      g_my_stat[M->my_hq] > 0 && (turn >= MAX_TURN - 3 || hq->hp <= HQ_LEVELS[HQ_MAX_LEVEL].hp / 2) && *budget >= HQ_HEAL_COST) {
    VEC_PUSH(a->upgrades, M->my_hq); *budget -= HQ_HEAL_COST;
  }
}

/* ---------------------------------------------------------------- army */
/* bodies kept at the HQ on top of its work slots: grows with the enemy army */
static int home_guard(const GameState *S, int turn) {
  int g = (turn < 100) ? V5_HOME_GUARD_EARLY : V5_HOME_GUARD_LATE;
  if (turn >= 120) g = max_int(g, enemy_alive_estimate(S) / 3);
  return min_int(g, 50);
}
static int rally_zone(const GameState *S, const GameMap *M) {
...
static int army_attack(Actions *a, const GameState *S, const GameMap *M, const Paths *P, int *budget, int turn, int rally) {
  /* stack = free bodies at rally beyond its work cap */
  ...
  /* keep the lowest-hp bodies as workers */
  ...
  stack_n -= keep;
  if (turn % 25 == 0) DBG("ARMY t=%d rally=%d stack=%d (keep %d) hqstat=%d\n", turn, rally, stack_n, keep, g_my_stat[M->my_hq]);
  if (stack_n < V5_ATTACK_MIN_STACK) return 0;
  /* outnumbered: stay defensive (the stack still gathers) unless the game is ending */
  if (turn < MAX_TURN - 60 && enemy_alive_estimate(S) > my_alive(S, M) * 11 / 10) return 0;
  /* candidate targets: known enemy bases, then HQ */
  ...
  for (int i = 0; i < S->buildings.len; ++i) {
    ...
    if (wl >= INF_HOPS) continue;
    /* defenders: everything visible in/next to the zone, PLUS every enemy
       warrior we cannot see right now (they may be sitting exactly there -
       the mirror match showed 16-stacks dying into an unseen 30-stack).
       Late in the game only half of the unseen are assumed to respond. */
    int dhp[512]; int dn = 0;
    int ehp = HQ_LEVELS[enemy_hq_level_estimate(S, M)].warrior_hp;
    for (int j = 0; j < S->warriors.len; ++j) {
      const Warrior *w = &S->warriors.data[j];
      if (w->id.side == M->my_side) continue;
      if (w->region == r || adjacent(M, w->region, r)) { if (dn < 512) dhp[dn++] = w->hp; }
    }
    ...
    }
    {
      ...
      if (turn >= MAX_TURN - 60) unseen /= 2;
      for (int j = 0; j < unseen && dn < 512; ++j) dhp[dn++] = ehp;
    }
    if (b->type == BTYPE_HQ) {
      /* plus what it can train while we walk: train_cap per turn (cheap bound) */
    ...
    int left = 0;
    int win = sim_zone(stack_hp, stack_n, 0, dhp, dn, building_turret(b), b->hp, 40, &left);
    int tot = 0; for (int j = 0; j < stack_n; ++j) tot += stack_hp[j];
    if (turn % 25 == 0) DBG("  target %d (%s lvl%d hp%d) defenders=%d win=%d left=%d/%d wl=%d\n", r, b->type == BTYPE_HQ ? "HQ" : "BASE", b->level, b->hp, dn, win, left, tot, wl);
    if (!win || left * 10 < tot * 6) continue;     /* need >= 60% of our hp left */
    if (wl < best_len || (b->type == BTYPE_HQ && !best_is_hq && wl <= best_len + 4)) { best = r; best_len = wl; best_is_hq = b->type == BTYPE_HQ; }
  }
  if (best < 0) return 0;
  /* launch: the whole stack at once (10 gold per body) - save up for it */
  int cost = MOVE_COST * stack_n;
  if (*budget < cost) { g_launch_reserve = cost; DBG("ATTACK t=%d saving %d for launch (have %d)\n", turn, cost, *budget); return 0; }
...
static void issue_training(Actions *a, const GameState *S, const GameMap *M, int *budget, int turn,
                           int open_slots, int want_claimer, int reserve) {
  const Building *hq = find_building_const(S, M->my_hq);
  ...
  int n = a->train_n;
  int wanted = open_slots + want_claimer + (g_role_scout < 0 && alive >= V5_SCOUT_MIN_ARMY - 1 ? 1 : 0);
  /* economy first */
  while (n < cap && wanted > 0) {
    if (*budget - TRAIN_COST < reserve) break;
    if (income - UPKEEP_PER_WARRIOR * (alive + n + 1) < 0) break;
    ++n; --wanted; *budget -= TRAIN_COST;
  }
  /* army: after the opening, whatever the economy sustains.  Upkeep is the
     hidden cost: keep net income at >= 50% of gross before turn 200 and
     >= 30% after, so bases/HQ keep upgrading. */
  if (turn >= V5_ARMY_TRAIN_START) {
    int share = (turn < 200) ? 50 : 30;
    int army_reserve = reserve + g_upgrade_reserve + g_launch_reserve;
    while (n < cap) {
      if (*budget - TRAIN_COST < army_reserve) break;
      int net = income - UPKEEP_PER_WARRIOR * (alive + n + 1);
      /* a big bank can carry upkeep: idle gold is worth nothing at turn 400 */
      int bank_ok = (*budget - TRAIN_COST > 1500) && (net >= -(*budget - 1000) / 60);
      if (net * 100 < income * share && !bank_ok) break;
      ++n; *budget -= TRAIN_COST;
    }
  }
  a->train_n = n;
...
static Actions decide(const GameState *S, const GameMap *M, const Paths *P, int turn) {
  ...
  ctx_scan(S, M); /* (positions unchanged; recompute nothing) */
  ...
  /* reserve: bases that will be built by arriving claimers soon */
  int reserve = 0;
  for (int i = 0; i < S->warriors.len; ++i) {
    const Warrior *w = &S->warriors.data[i];
    if (w->id.side != M->my_side) continue;
  ...
  int rally = rally_zone(S, M);
  /* an enemy force on our half: the army stays home until it is gone */
  {
    int half_hp = 0;
    for (int i = 0; i < S->warriors.len; ++i) {
      const Warrior *w = &S->warriors.data[i];
  ...
  if (!hq_threat) issue_claims(&a, S, M, P, &budget, income_net, &want_claimer, turn);
  issue_upgrades(&a, S, M, &budget, turn, reserve);

  /* 6. idle bodies to the rally */
  if (!hq_threat && turn >= 40) army_to_rally(&a, S, M, &budget, turn, rally);

  /* 7. training */
  issue_training(&a, S, M, &budget, turn, open_slots, want_claimer, reserve);
