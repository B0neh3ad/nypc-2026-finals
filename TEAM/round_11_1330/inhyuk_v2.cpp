/* INHYUK V2 (2026-08-29)
   Based on the behavior-preserving inhyuk_v1 refactor.  Adds one bounded,
   expendable forward observer chosen for legal two-hop vision gain. */
...
#endif
#ifndef ENABLE_DEEP_VISION_SCOUT
#define ENABLE_DEEP_VISION_SCOUT 1
#endif
#ifndef VISION_SCOUT_START_TURN
#define VISION_SCOUT_START_TURN 52
#endif
#ifndef VISION_SCOUT_MIN_OWN_WARRIORS
#define VISION_SCOUT_MIN_OWN_WARRIORS 6
#endif
#ifndef VISION_SCOUT_MIN_OWN_BASES
#define VISION_SCOUT_MIN_OWN_BASES 2
#endif
#ifndef VISION_SCOUT_MIN_ROUTE_HOPS
#define VISION_SCOUT_MIN_ROUTE_HOPS 5
#endif
#ifndef VISION_SCOUT_MIN_NEW_REGIONS
#define VISION_SCOUT_MIN_NEW_REGIONS 10
#endif
...
static int g_vision_scout_launched = 0;
...
/* A scout is useful only when it sees territory that our economy and ordinary
   expansion do not already cover.  Score the exact movement route using the
   game's legal two-hop vision, with extra weight on newly exposed strongholds
   because hidden enemy bases were the largest blind spot in the Round-7 logs. */
static int vision_scout_route_gain(const GameState *S, const GameMap *M,
                                   const Paths *P, int source, int target,
                                   int *new_regions_out,
                                   int *new_strongholds_out) {
  int already_visible[256] = {0};
  int newly_seen[256] = {0};
  int stronghold_mask[256] = {0};
  for (int r = 0; r < M->N; ++r)
    already_visible[r] = region_visible_to_me(S, M, r);
  for (int i = 0; i < M->strongholds.len; ++i)
    stronghold_mask[M->strongholds.data[i]] = 1;

  int new_regions = 0, new_strongholds = 0;
  int cur = source, guard = 0;
  while (cur != target && guard++ <= M->N + 5) {
    cur = P->nxt[cur][target];
    if (cur < 0 || cur >= M->N) return 0;
    for (int r = 0; r < M->N; ++r) {
      if (already_visible[r] || newly_seen[r]) continue;
      if (!region_within_two_hops(M, cur, r)) continue;
      newly_seen[r] = 1;
      ++new_regions;
      if (stronghold_mask[r]) ++new_strongholds;
    }
  }
  if (cur != target) return 0;
  if (new_regions_out != NULL) *new_regions_out = new_regions;
  if (new_strongholds_out != NULL) *new_strongholds_out = new_strongholds;
  return 1;
}

static int vision_scout_target_is_committed(const GameState *S,
                                            const GameMap *M,
                                            const Actions *a, int target) {
  for (int i = 0; i < S->warriors.len; ++i) {
    const Warrior *w = &S->warriors.data[i];
    if (w->id.side == M->my_side && w->state == WSTATE_MOVING &&
        w->target == target)
      return 1;
  }
  for (int i = 0; i < a->moves.len; ++i)
...
static int vision_scout_preserves_next_train(const GameState *S,
                                             const GameMap *M,
                                             const Actions *a, int budget) {
  if (budget < MOVE_COST) return 0;
  int train_room = planned_train_cap(S, M, a) - a->train_n;
  if (train_room <= 0) return 1;
  return budget / TRAIN_COST == (budget - MOVE_COST) / TRAIN_COST;
}

static int vision_scout_choose_safe_source(const GameState *S,
                                           const GameMap *M,
                                           const Paths *P,
                                           const Actions *a, int target,
                                           WarriorId *id_out) {
  double best_dist = INFINITY;
  int best_hp = 1000000000;
  WarriorId best = {M->my_side, -1};
  for (int bi = 0; bi < S->buildings.len; ++bi) {
    const Building *b = &S->buildings.data[bi];
    if (b->side != M->my_side || P->nxt[b->region][target] < 0) continue;
    int keep = planned_work_cap_at(S, M, a, b->region);
    if (b->type == BTYPE_HQ) keep = max_int(keep, 3);
    if (planned_workers_physically_remaining_at(S, a, M->my_side,
                                                b->region) <= keep)
      continue;
    for (int wi = 0; wi < S->warriors.len; ++wi) {
      const Warrior *w = &S->warriors.data[wi];
      if (w->id.side != M->my_side || w->region != b->region ||
        ...
        continue;
      if (region_has_enemy_warrior(S, M, w->region)) continue;
      double dist = P->dist[w->region][target];
      if (best.num < 0 || dist < best_dist ||
          (dist == best_dist && w->hp < best_hp)) {
        best = w->id;
        best_dist = dist;
        best_hp = w->hp;
      }
...
/* Re-purpose one move that the opening would otherwise spend on a nearby
   neutral: send the same surplus body toward an enemy-half stronghold along a
   route that reveals substantial unseen territory.
   There is no extra training cost and no worker slot is vacated.  If it lives,
   it stays as a forward observer/builder; if it meets a hidden army or base,
   the contact itself is the warning we paid for. */
static int issue_forward_vision_claim(Actions *a, const GameState *S,
                                      const GameMap *M, const Paths *P,
                                      int *budget, int turn) {
#if !ENABLE_DEEP_VISION_SCOUT
  (void)a; (void)S; (void)M; (void)P; (void)budget; (void)turn;
  return 0;
#else
  if (g_vision_scout_launched || turn < VISION_SCOUT_START_TURN ||
      turn > OPENING_NEUTRAL_FIRST_MAX_TURN)
    return 0;
  if (count_side_warriors(S, M->my_side) < VISION_SCOUT_MIN_OWN_WARRIORS ||
      normal_base_count_for_side(S, M->my_side) < VISION_SCOUT_MIN_OWN_BASES)
    return 0;
  if (g_army_deficit > 0 || turn <= g_early_mass_contact_until_turn) return 0;
  if (all_regions_visible_to_me(S, M)) return 0;
  if (!vision_scout_preserves_next_train(S, M, a, *budget)) return 0;

  int best_target = -1, best_gain = -1, best_regions = -1;
  int best_hops = INF_HOPS;
  WarriorId best_id = {M->my_side, -1};
  for (int si = 0; si < M->strongholds.len; ++si) {
    int target = M->strongholds.data[si];
    if (find_building_const(S, target) != NULL ||
        vision_scout_target_is_committed(S, M, a, target))
      continue;
    if (!(P->dist[target][M->opp_hq] < P->dist[target][M->my_hq]))
      continue;

    WarriorId id;
    if (!vision_scout_choose_safe_source(S, M, P, a, target, &id)) continue;
    const Warrior *w = find_warrior_const(S, id);
    if (w == NULL || full_path_enemy_blocked(S, M, P, w->region, target, 1))
      continue;
    int hops = path_hops_between(P, w->region, target);
    if (hops < VISION_SCOUT_MIN_ROUTE_HOPS || hops >= INF_HOPS) continue;

    int new_regions = 0, new_strongholds = 0;
    if (!vision_scout_route_gain(S, M, P, w->region, target,
                                 &new_regions, &new_strongholds) ||
        new_regions < VISION_SCOUT_MIN_NEW_REGIONS)
      continue;
    int gain = new_regions + 12 * new_strongholds;
    if (!region_visible_to_me(S, M, target)) gain += 8;
    if (best_target < 0 || gain > best_gain ||
        (gain == best_gain && new_regions > best_regions) ||
        (gain == best_gain && new_regions == best_regions && hops < best_hops)) {
      best_target = target;
      best_gain = gain;
  ...
  if (!add_move_action_ex_stack_flags(a, S, M, P, best_id, best_target,
                                      budget, flags))
    return 0;
  g_vision_scout_launched = 1;
#ifdef DEBUG_PATCH
  const Warrior *chosen = find_warrior_const(S, best_id);
  fprintf(stderr,
          "vision-claim t=%d id=%d from=%d waypoint=%d hops=%d new=%d gain=%d\n",
          turn, best_id.num, chosen != NULL ? chosen->region : -1,
          best_target, best_hops, best_regions, best_gain);
#endif
  return 1;
#endif
}
...
static Actions decide(const GameState *S, const GameMap *M, const Paths *P,
  ...
  if (!defense_plan.triggered && !enemy_stack_attack_triggered)
    issue_forward_vision_claim(&a, S, M, P, &budget, turn);
