...
#endif
#ifndef ENABLE_POTENTIAL_RUSH_HQ_RESERVE
#define ENABLE_POTENTIAL_RUSH_HQ_RESERVE 1
#endif
...
#define POTENTIAL_RUSH_MAX_HOPS 12
#endif
#ifndef POTENTIAL_RUSH_RESERVE_MARGIN
#define POTENTIAL_RUSH_RESERVE_MARGIN 2
#endif
#ifndef POTENTIAL_RUSH_MEMORY_TURNS
#define POTENTIAL_RUSH_MEMORY_TURNS 6
#endif
#ifndef POTENTIAL_RUSH_RECALL_MAX_HOPS
#define POTENTIAL_RUSH_RECALL_MAX_HOPS 12
#endif
...
static int g_potential_rush_reserve_units = 0;
static int g_potential_rush_reserve_until_turn = -1;
...
static PotentialRushThreat potential_rush_visible_cluster(const GameState *S,
                                                           const GameMap *M,
                                                           const Paths *P) {
  PotentialRushThreat best;
  best.count = 0;
  best.hp = 0;
  best.center_region = -1;
  best.hq_hops = INF_HOPS;

  Side opp = opposite(M->my_side);
  for (int i = 0; i < S->warriors.len; ++i) {
    const Warrior *c = &S->warriors.data[i];
    if (c->id.side != opp) continue;
    int ch = path_hops_between(P, M->my_hq, c->region);
    if (ch >= INF_HOPS) continue;
    if (POTENTIAL_RUSH_MAX_HOPS >= 0 && ch > POTENTIAL_RUSH_MAX_HOPS) continue;

    int cnt = 0, hp = 0, min_h = INF_HOPS;
    for (int j = 0; j < S->warriors.len; ++j) {
      const Warrior *w = &S->warriors.data[j];
      if (w->id.side != opp) continue;
      int cluster_h = path_hops_between(P, c->region, w->region);
      if (cluster_h >= INF_HOPS || cluster_h > POTENTIAL_RUSH_CLUSTER_RADIUS) continue;
      int hh = path_hops_between(P, M->my_hq, w->region);
      if (hh >= INF_HOPS) continue;
      if (POTENTIAL_RUSH_MAX_HOPS >= 0 && hh > POTENTIAL_RUSH_MAX_HOPS) continue;
      ++cnt;
      hp += max_int(1, w->hp);
      if (hh < min_h) min_h = hh;
    }
    if (cnt > best.count ||
        (cnt == best.count && hp > best.hp) ||
        (cnt == best.count && hp == best.hp && min_h < best.hq_hops)) {
      best.count = cnt;
      best.hp = hp;
      best.center_region = c->region;
      best.hq_hops = min_h;
    }
  }
  return best;
...
static int potential_rush_cluster_holds_with_defenders(const GameState *S,
                                                        const GameMap *M,
                                                        const Paths *P,
                                                        PotentialRushThreat threat,
                                                        int defender_count) {
  const Building *hq = find_building_const(S, M->my_hq);
  if (hq == NULL || hq->side != M->my_side || hq->type != BTYPE_HQ) return 0;
  if (threat.center_region < 0 || threat.count <= 0) return 1;

  int atk_hp[MAX_COMBAT_SIM_UNITS];
  int def_hp[MAX_COMBAT_SIM_UNITS];
  int an = 0;
  Side opp = opposite(M->my_side);
  for (int i = 0; i < S->warriors.len && an < MAX_COMBAT_SIM_UNITS; ++i) {
    const Warrior *w = &S->warriors.data[i];
    if (w->id.side != opp) continue;
    int cluster_h = path_hops_between(P, threat.center_region, w->region);
    if (cluster_h >= INF_HOPS || cluster_h > POTENTIAL_RUSH_CLUSTER_RADIUS) continue;
    int hh = path_hops_between(P, M->my_hq, w->region);
    if (hh >= INF_HOPS) continue;
    if (POTENTIAL_RUSH_MAX_HOPS >= 0 && hh > POTENTIAL_RUSH_MAX_HOPS) continue;
    atk_hp[an++] = max_int(1, w->hp);
  }
  if (an <= 0) return 1;

  int dn = min_int(defender_count, MAX_COMBAT_SIM_UNITS);
  int whp = HQ_LEVELS[hq->level].warrior_hp;
  for (int i = 0; i < dn; ++i) def_hp[i] = whp;
  int building_hp = max_int(1, hq->hp);
  int turret = building_turret_power_const(hq);

  for (int day = 0; day < 80; ++day) {
    int alive_a = 0, alive_d = 0;
    for (int i = 0; i < an; ++i) if (atk_hp[i] > 0) ++alive_a;
    for (int i = 0; i < dn; ++i) if (def_hp[i] > 0) ++alive_d;
    if (alive_a <= 0) return 1;

    for (int k = 0; k < turret; ++k) {
      int bi = -1;
      for (int i = 0; i < an; ++i)
        if (atk_hp[i] > 0 && (bi < 0 || atk_hp[i] < atk_hp[bi])) bi = i;
      if (bi >= 0) --atk_hp[bi];
    }

    for (int k = 0; k < alive_a; ++k) {
      int bi = -1;
      for (int i = 0; i < dn; ++i)
        if (def_hp[i] > 0 && (bi < 0 || def_hp[i] < def_hp[bi])) bi = i;
      if (bi >= 0) --def_hp[bi];
      else --building_hp;
    }

    for (int k = 0; k < alive_d; ++k) {
      int bi = -1;
      for (int i = 0; i < an; ++i)
        if (atk_hp[i] > 0 && (bi < 0 || atk_hp[i] < atk_hp[bi])) bi = i;
      if (bi >= 0) --atk_hp[bi];
...
static int potential_rush_required_hq_reserve(const GameState *S,
                                               const GameMap *M,
                                               const Paths *P,
                                               PotentialRushThreat threat,
                                               const Actions *a) {
  if (threat.count < POTENTIAL_RUSH_MIN_CLUSTER) return 0;
  int max_try = min_int(MAX_COMBAT_SIM_UNITS,
                        threat.count + POTENTIAL_RUSH_RESERVE_MARGIN + 24);
  int required = max_try;
  for (int n = 0; n <= max_try; ++n) {
    if (potential_rush_cluster_holds_with_defenders(S, M, P, threat, n)) {
      required = n;
      break;
    }
  }
  required += POTENTIAL_RUSH_RESERVE_MARGIN;
...
static int potential_rush_hq_reserve_update(const GameState *S,
                                             const GameMap *M,
                                             const Paths *P,
                                             const Actions *a,
                                             int turn) {
#if ENABLE_POTENTIAL_RUSH_HQ_RESERVE
  PotentialRushThreat threat = potential_rush_visible_cluster(S, M, P);
  int required = potential_rush_required_hq_reserve(S, M, P, threat, a);
  if (required > 0) {
    if (required > g_potential_rush_reserve_units)
      g_potential_rush_reserve_units = required;
    g_potential_rush_reserve_until_turn = turn + POTENTIAL_RUSH_MEMORY_TURNS;
  } else if (turn > g_potential_rush_reserve_until_turn) {
    g_potential_rush_reserve_units = 0;
  }
#ifdef DEBUG_POTENTIAL_RUSH_RESERVE
  fprintf(stderr, "# RESERVE t=%d visible_cluster=%d hp=%d center=%d hops=%d required=%d held=%d until=%d\n",
          turn, threat.count, threat.hp, threat.center_region, threat.hq_hops,
          required, g_potential_rush_reserve_units, g_potential_rush_reserve_until_turn);
#endif
  return g_potential_rush_reserve_units;
#else
...
static int issue_potential_rush_hq_reserve_fill(Actions *a,
                                                 const GameState *S,
                                                 const GameMap *M,
                                                 const Paths *P,
                                                 int *budget) {
#if ENABLE_POTENTIAL_RUSH_HQ_RESERVE
  int target = g_potential_rush_reserve_units;
  if (target <= 0) return 0;

  int current = direct_hq_current_defender_stats(S, M, a, NULL);
  int shortage = target - current;
  if (shortage <= 0) return 0;

  DirectHqDefenderCandidate cands[MAX_COMBAT_SIM_UNITS];
  int cn = direct_hq_collect_recall_candidates(S, M, P, a, cands,
                                               MAX_COMBAT_SIM_UNITS);
  int issued = 0;
  for (int i = 0; i < cn && shortage > 0; ++i) {
    if (POTENTIAL_RUSH_RECALL_MAX_HOPS >= 0 &&
        cands[i].eta > POTENTIAL_RUSH_RECALL_MAX_HOPS) continue;

    const Building *src = find_building_const(S, cands[i].region);
    if (src != NULL && src->side == M->my_side) {
      int remain = planned_workers_physically_remaining_at(S, a, M->my_side,
                                                            cands[i].region);
      int floor = max_int(1, planned_work_cap_at(S, M, a, cands[i].region));
      if (remain <= floor) continue;
    }
  ...
  if (shortage > 0 && a->train_n == 0) {
    int cap = min_int(planned_train_cap(S, M, a), *budget / TRAIN_COST);
    int train = min_int(shortage, cap);
    if (train > 0) {
      a->train_n = train;
      *budget -= TRAIN_COST * train;
      issued += train;
      shortage -= train;
    }
  }
#ifdef DEBUG_POTENTIAL_RUSH_RESERVE
  fprintf(stderr, "# RESERVE_FILL t=%d target=%d current=%d issued=%d shortage_after=%d\n",
          g_current_turn, target, current, issued, shortage);
#endif
  return issued;
#else
  (void)a; (void)S; (void)M; (void)P; (void)budget;
...
static Actions decide(const GameState *S, const GameMap *M, const Paths *P,
  ...
  g_home_defense_forced_hq_keep = max_int(
      g_home_defense_forced_hq_keep,
      potential_rush_hq_reserve_update(S, M, P, &a, turn));
  issue_potential_rush_hq_reserve_fill(&a, S, M, P, &budget);
