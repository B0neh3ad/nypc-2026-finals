/* staged_rush 변형: 8명 파티(T6) + 증원 */
#define RUSH_TRAIN_TURNS 6
#define RUSH_LAUNCH_TURN 6
#define RUSH_FORWARD_REINF 1
#include <algorithm>
#include <cassert>
#include <cmath>
#include <cstdlib>
...
#endif
#ifndef RUSH_WAVE_SIZE
#define RUSH_WAVE_SIZE 1 /* 증원을 몇 명 모아 보낼지 */
#endif
...
static Actions decide(const GameState &S, const GameMap &M, const Paths &P,
  ...
  if (turn > RUSH_LAUNCH_TURN) {
    ...
    /* 재돌격 이후 경유지에 늦게 도착한 병력(낙오·증원)은 계속 본부로 흘려보낸다.
       이게 없으면 경유지에 영영 서 있는다 — rev1 의 실제 버그. */
    if (g_redirected && g_stage_region >= 0)
      for (const auto &w : S.warriors)
        if (w.id.side == M.my_side && w.hp > 0 &&
            w.state == WState::STATIONARY && w.region == g_stage_region)
          a.moves.emplace_back(w.id, M.opp_hq);

#if RUSH_FORWARD_REINF
    /* 증원: 본부 노동 인원만 남기고 전선으로. RUSH_WAVE_SIZE 명이 모일 때까지
       본부에서 대기시켰다가 묶어서 보낸다 (1이면 스트림). */
    int home = 0;
    ...
    if ((int)spare.size() >= RUSH_WAVE_SIZE)
