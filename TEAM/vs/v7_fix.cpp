...
/* ===== 보완 로직 손잡이 (2026-08-29) ================================== */
#ifndef ENABLE_STAGED_RUSH_WATCH
#define ENABLE_STAGED_RUSH_WATCH 1
#endif
#ifndef STAGED_WATCH_RADIUS      /* 본부에서 이 홉 안을 감시 */
#define STAGED_WATCH_RADIUS 4
#endif
#ifndef STAGED_WATCH_COUNT       /* 이 수 이상 뭉쳐 있으면 경보 */
#define STAGED_WATCH_COUNT 4
#endif
#ifndef STAGED_WATCH_MAX_TURN    /* 경보를 볼 구간 (초반 러시 전용) */
#define STAGED_WATCH_MAX_TURN 60
#endif
#ifndef STAGED_WATCH_HOLD_TURNS  /* 한 번 걸리면 유지할 턴 */
#define STAGED_WATCH_HOLD_TURNS 20
#endif
#ifndef STAGED_WATCH_MIN_GARRISON
#define STAGED_WATCH_MIN_GARRISON 5
#endif

#ifndef ENABLE_NO_SPEND_UNDER_FIRE
#define ENABLE_NO_SPEND_UNDER_FIRE 1
#endif
#ifndef UNDER_FIRE_ADJ_STACK     /* 인접에 이만큼 뭉쳐 있어도 위험으로 본다 */
#define UNDER_FIRE_ADJ_STACK 3
#endif
/* ====================================================================== */
...
#define LATE_HQ_PROBE_START_TURN 330
#endif
#ifndef LATE_HQ_PROBE_MIN_ARMY
#define LATE_HQ_PROBE_MIN_ARMY 30
#endif
...
#define STRATEGIC_HOLD_START_TURN 140
#endif
#ifndef STRATEGIC_HOLD_STACK_RADIUS
#define STRATEGIC_HOLD_STACK_RADIUS 8
#endif
#ifndef STRATEGIC_HOLD_STACK_THRESHOLD
#define STRATEGIC_HOLD_STACK_THRESHOLD 12
#endif
#ifndef STRATEGIC_HOLD_ARMY_SHARE_DEN
#define STRATEGIC_HOLD_ARMY_SHARE_DEN 2
#endif
...
/* ===== 보완 로직 구현 ================================================== */
static int g_staged_watch_until = -1;
static const GameState *g_under_fire_S = NULL;
static const GameMap *g_under_fire_M = NULL;

/* 본부 반경 안에 적이 뭉쳐 있는가. staged rush 는 본부가 아니라 인근 거점에
   모였다가 재돌격하므로, '본부로 오는 중'이 아니라 '근처에 서 있는 것'을 본다. */
static int staged_rush_threat(const GameState *S, const GameMap *M, const Paths *P,
                              int turn) {
#if !ENABLE_STAGED_RUSH_WATCH
    (void)S; (void)M; (void)P; (void)turn; return 0;
#else
    if (turn > STAGED_WATCH_MAX_TURN) return turn <= g_staged_watch_until;
    int seen = 0;
    for (int i = 0; i < S->warriors.len; ++i) {
        const Warrior *w = &S->warriors.data[i];
        ...
        if (path_hops_between(P, M->my_hq, r) <= STAGED_WATCH_RADIUS) ++seen;
    }
    if (seen >= STAGED_WATCH_COUNT)
        g_staged_watch_until = turn + STAGED_WATCH_HOLD_TURNS;
    return turn <= g_staged_watch_until;
#endif
}

/* 적이 붙어 있는(또는 인접에 뭉쳐 있는) 건물인가. 그런 건물에 돈을 쓰면
   다음 웨이브가 그대로 태운다 — 빔앤권전에서 T100~150 에 3,400g 을 그렇게 잃었다. */
static int region_under_fire(const GameState *S, const GameMap *M, int region) {
#if !ENABLE_NO_SPEND_UNDER_FIRE
    (void)S; (void)M; (void)region; return 0;
#else
    if (region < 0 || region >= M->N) return 0;
    if (region_has_enemy_warrior(S, M, region)) return 1;
    int adj_enemy = 0;
    for (int j = 0; j < M->adj[region].len; ++j)
        adj_enemy += enemy_warrior_count_at(S, M, M->adj[region].data[j]);
    return adj_enemy >= UNDER_FIRE_ADJ_STACK;
#endif
}
/* ====================================================================== */
...
static int add_upgrade_action(Actions *a, int region) {
  ...
  /* 적이 붙어 있는 건물에는 돈을 쓰지 않는다. 다음 웨이브가 그대로 태운다.
     본부는 예외 — 본부 수리를 막으면 그대로 진다. */
  if (g_under_fire_S != NULL && g_under_fire_M != NULL &&
      region != g_under_fire_M->my_hq &&
      region_under_fire(g_under_fire_S, g_under_fire_M, region))
    return 0;
...
static Actions decide(const GameState *S, const GameMap *M, const Paths *P,
  ...
  g_home_defense_forced_hq_keep =
      (turn <= g_early_mass_contact_until_turn)
          ...
          : 0;
  /* staged rush: 본부 근처에 적이 뭉쳐 '서 있는' 것만으로 주둔을 끌어올린다.
     기존 EARLY_MASS_CONTACT 는 피해를 입은 뒤 발동이라 T25 급사를 못 막는다. */
  if (staged_rush_threat(S, M, P, turn) &&
      g_home_defense_forced_hq_keep < STAGED_WATCH_MIN_GARRISON)
    g_home_defense_forced_hq_keep = STAGED_WATCH_MIN_GARRISON;
