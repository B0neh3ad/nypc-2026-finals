...
#endif
#ifndef ENABLE_DYNAMIC_HQ_DEFENSE_RESERVE
#define ENABLE_DYNAMIC_HQ_DEFENSE_RESERVE 1
#endif
#ifndef DYNAMIC_HQ_RESERVE_MIN_THREAT
#define DYNAMIC_HQ_RESERVE_MIN_THREAT 3
#endif
#ifndef DYNAMIC_HQ_RESERVE_MARGIN
#define DYNAMIC_HQ_RESERVE_MARGIN 2
#endif
#ifndef DYNAMIC_HQ_RESERVE_MEMORY_TURNS
#define DYNAMIC_HQ_RESERVE_MEMORY_TURNS 4
#endif
...
static int g_dynamic_hq_reserve_units = 0;
static int g_dynamic_hq_reserve_until_turn = -1;
...
static int dynamic_hq_defense_reserve_update(const GameState *S,
                                             const GameMap *M,
                                             const Paths *P,
                                             const Actions *a,
                                             int turn) {
#if ENABLE_DYNAMIC_HQ_DEFENSE_RESERVE
  DirectHqThreat threat = direct_hq_threat_stats(S, M, P);
  if (threat.count >= DYNAMIC_HQ_RESERVE_MIN_THREAT) {
    int required = threat.count + DYNAMIC_HQ_RESERVE_MARGIN;
#if HOME_DEFENSE_KEEP_WORKERS
    required = max_int(required, planned_work_cap_at(S, M, a, M->my_hq));
#endif
    if (required > g_dynamic_hq_reserve_units)
      g_dynamic_hq_reserve_units = required;
    g_dynamic_hq_reserve_until_turn = turn + DYNAMIC_HQ_RESERVE_MEMORY_TURNS;
  } else if (turn > g_dynamic_hq_reserve_until_turn) {
    g_dynamic_hq_reserve_units = 0;
  }
  return g_dynamic_hq_reserve_units;
#else
...
static Actions decide(const GameState *S, const GameMap *M, const Paths *P,
  ...
  g_home_defense_forced_hq_keep = max_int(
      g_home_defense_forced_hq_keep,
      dynamic_hq_defense_reserve_update(S, M, P, &a, turn));
