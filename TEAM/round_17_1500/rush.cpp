...
//////////////////////////////////
/* ===== RUSH OPENING (2026-08-29) ==========================================
   중간평가에서 우리를 반복해서 잡은 3팀(Re:Entropy, 안녕하세요민이…,
   고양이와 집사)의 오프닝을 재현한 스파링 상대.

   대회 로그에서 뽑은 실제 명령 시퀀스 (round_4/battle_676265, round_2/675855):

       T1..T5  TRAIN 1                    매턴 1명. 초기 3명 + 5 = 8명
       T6      MOVE <7명> <적 본부>        1명만 남기고 전원 올인
       이후    TRAIN 1 (금화 되는 대로)     T29,40,53,71,94,134,253,379 …

   경제가 이 타이밍을 강제한다. 시작 750, 훈련 5×120 = 600, 이동 7×10 = 70.
   6턴이면 사실상 파산이고, 본부에 남긴 1명의 노동 15/일로 8명 식비 16/일을
   겨우 감당한다. 그래서 이후 훈련 간격이 점점 벌어진다.

   적 본부까지 도보 약 19턴이라 25턴경 도착한다. 상대가 확장만 하고 있으면
   본부 체력 10은 5턴 만에 무너진다.

   파라미터는 상단 상수로 뺐다 — 러시 변형(더 이르게/늦게, 병력 수 조절)을
   만들어 방어 파라미터를 튜닝할 때 쓰라고.
   ========================================================================= */

/* 러시 튜닝 파라미터 */
#ifndef RUSH_TRAIN_TURNS
#define RUSH_TRAIN_TURNS 5 /* 몇 턴 동안 매턴 1명씩 뽑나 */
#endif
#ifndef RUSH_LAUNCH_TURN
#define RUSH_LAUNCH_TURN 6 /* 몇 턴에 출격하나 */
#endif
#ifndef RUSH_KEEP_HOME
#define RUSH_KEEP_HOME 1 /* 본부에 몇 명 남기나 (노동용) */
#endif
#ifndef RUSH_REINFORCE
#define RUSH_REINFORCE 1 /* 출격 후에도 금화 되는 대로 계속 뽑나 */
#endif
#ifndef RUSH_SEND_REINFORCEMENTS
#define RUSH_SEND_REINFORCEMENTS 0 /* 새로 뽑은 병력도 적 본부로 보내나 */
#endif
...
static Actions decide(const GameState &S, const GameMap &M, const Paths &P,
  ...
  if (turn == RUSH_LAUNCH_TURN) {
    ...
    for (const Warrior *w : mine) {
      /* 본부에 있는 병사부터 keep 만큼 남긴다 (노동 수입원). */
      if (keep > 0 && w->region == my_hq) {
        --keep;
        continue;
      }
  ...
  /* 3) 이후: 금화가 되면 계속 뽑는다. 기본은 본부에 두어 노동시킨다.
        (실제 로그의 T29,40,53,… 간헐 훈련이 이 동작에서 나온다) */
  if (RUSH_REINFORCE && turn > RUSH_LAUNCH_TURN) {
    if (S.gold >= TRAIN_COST)
      a.train_n = 1;
#if RUSH_SEND_REINFORCEMENTS
    for (const auto &w : S.warriors) {
      if (w.id.side != M.my_side || w.hp <= 0)
        continue;
