/* INHYUK V4 FINAL (2026-08-29): contest-tested liveness, scouting, and finishers. */
...
#define POST_ATTACK_MAX_NEUTRAL_WAITERS 2
#endif
/* Expansion is allowed to run with this many unfilled work slots outstanding.
   Requiring zero (the old behaviour) let a single vacancy anywhere on the map
   freeze all expansion. */
#ifndef POST_ATTACK_WORK_SLOT_SLACK
#define POST_ATTACK_WORK_SLOT_SLACK 1
#endif

/* Hold non-HQ upgrades while strongholds are still unclaimed, up to this turn.
   Bounded so that a stronghold held by an enemy we can no longer see cannot
   suppress base upgrades for the rest of the game. */
#ifndef EXPAND_FIRST_UNTIL_TURN
#define EXPAND_FIRST_UNTIL_TURN 260
#endif
...
/* A new base costs 500 and buys a work slot, a turret, two hops of vision, a
   free move anchor and denial of the site to the enemy.  Upgrading an existing
   base costs 550/600 and buys the work slot alone.  So while any stronghold is
   unclaimed, building strictly dominates upgrading.  Worse, each upgrade opens
   a fresh empty work slot, which post_attack_owned_work_slots_full() reads as
   "not ready to expand" - in round 12 the two effects together froze our
   expansion at 4 bases from turn ~50 to ~135 in every game we did not win. */
static int expansion_stronghold_remains(const GameState *S, const GameMap *M) {
  for (int i = 0; i < M->strongholds.len; ++i) {
    int r = M->strongholds.data[i];
    const Building *b = find_building_const(S, r);
    if (b == NULL) return 1;               /* unbuilt, or not currently seen */
  }
  return 0;
}
...
static int issue_existing_upgrades(Actions *a, const GameState *S,
  ...
  while (1) {
    ...
    if (b->type != BTYPE_HQ && g_current_turn < EXPAND_FIRST_UNTIL_TURN &&
        expansion_stronghold_remains(S, M)) break;
...
static int post_attack_owned_work_slots_full(const GameState *S,
  ...
  return deficit <= POST_ATTACK_WORK_SLOT_SLACK;
#endif
}
...
static int issue_deadlock_vision_scout(Actions *a, const GameState *S,
  ...
  for (int i = 0; i < M->strongholds.len; ++i) {
    ...
    long long score = (g_remembered_enemy_base[r] ? 1000000000000000LL : 0LL) +
