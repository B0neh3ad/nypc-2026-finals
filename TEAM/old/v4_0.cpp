...
static int P_ENDGAME_SYNC_START_TURN = 172 + LATE_TURN_SHIFT;   /* MoE runtime param (end-anchored) */
#define ENDGAME_SYNC_START_TURN P_ENDGAME_SYNC_START_TURN
#endif
#ifndef ENDGAME_SYNC_ARRIVAL_TURN
static int P_ENDGAME_SYNC_ARRIVAL_TURN = 190 + LATE_TURN_SHIFT;   /* MoE runtime param (end-anchored) */
...
static int P_LATE_TIEBREAK_START_TURN = 152 + LATE_TURN_SHIFT;   /* MoE runtime param (end-anchored) */
...
static int P_LATE_SAVER_START_TURN = 140 + LATE_TURN_SHIFT;   /* MoE runtime param (end-anchored) */
...
static int P_FINISHER_START_TURN = 150 + LATE_TURN_SHIFT;   /* MoE runtime param (end-anchored) */
...
static int P_CASH_DUMP_TRAIN_START_TURN = 116 + LATE_TURN_SHIFT;   /* MoE runtime param (end-anchored) */
...
static int P_FINAL_HQ5_CASH_DUMP_START_TURN = 169 + LATE_TURN_SHIFT;   /* MoE runtime param (end-anchored) */
#define FINAL_HQ5_CASH_DUMP_START_TURN P_FINAL_HQ5_CASH_DUMP_START_TURN
#endif
#ifndef FINAL_HQ5_HARD_DUMP_START_TURN
static int P_FINAL_HQ5_HARD_DUMP_START_TURN = 198 + LATE_TURN_SHIFT;   /* MoE runtime param (end-anchored) */
...
static int count_side_warriors(const GameState *S, Side side) {
  /* NEXT VISION: for the enemy this is an ESTIMATE (fog), not a visible count.
     Positional logic must use count_warriors_at() instead. */
  if (side != S->my_side) return enemy_alive_estimate(S);
  int cnt = 0;
  for (int i = 0; i < S->warriors.len; ++i)
...
static int side_total_warrior_hp(const GameState *S, Side side) {
  int hp = 0;
  if (side != S->my_side) {
    /* NEXT VISION: estimate (fog); GameMap is not available here but only
       my_hq/opp_hq are needed, so rebuild a minimal view. */
    int vis_hp = 0, vis_n = 0, max_hp = 0;
    for (int i = 0; i < S->warriors.len; ++i) {
      const Warrior *w = &S->warriors.data[i];
    ...
    for (int i = 0; i < S->buildings.len; ++i) {
      const Building *b = &S->buildings.data[i];
      if (b->side != S->my_side && b->type == BTYPE_HQ) lvl = b->level;
    }
    int implied = g_ei_max_hp_seen - HQ_LEVELS[1].warrior_hp + 1;
    if (implied > HQ_MAX_LEVEL) implied = HQ_MAX_LEVEL;
    if (implied > lvl) lvl = implied;
    max_hp = HQ_LEVELS[lvl].warrior_hp;
    int unseen = enemy_alive_estimate(S) - vis_n;
    if (unseen < 0) unseen = 0;
    return vis_hp + unseen * max_hp;
  }
  for (int i = 0; i < S->warriors.len; ++i) {
    const Warrior *w = &S->warriors.data[i];
...
static int side_current_income(const GameState *S, Side side) {
  ...
  for (int bi = 0; bi < S->buildings.len; ++bi) {
    ...
    int cap = building_work_cap(b);
    int workers;
    if (side != S->my_side && !region_visible(S, b->region))
      workers = cap; /* NEXT VISION: unseen enemy building -> assume staffed */
    else
      workers = count_warriors_at(S, side, b->region);
...
static int enemy_normal_bases_cleared_now(const GameState *S, const GameMap *M) {
  /* NEXT VISION: unknown != cleared (this gate used to block all of our own
     base upgrades from turn 1 because no enemy base had been *seen* yet). */
  if (!all_strongholds_observed(S, M)) return 0;
...
static int late_saver_hq_threat_now(const GameState *S, const GameMap *M,
  ...
  for (int i = 0; i < S->warriors.len; ++i) {
    const Warrior *w = &S->warriors.data[i];
    if (w->id.side != opp) continue;
    if (w->state == WSTATE_MOVING && w->target == M->my_hq) { ++near; continue; }
...
static int g_prev_turn[2][DEFENSE_MAX_TRACKED_ID]; /* fog: when it was last seen */
...
static int previous_region_of(WarriorId id) {
  ...
  /* Fog of war: an enemy that dropped out of vision and reappeared later has
     a stale "previous" position; only a position from the immediately
     preceding turn says anything about its heading. */
  if (g_prev_turn[side_index(id.side)][id.num] != g_current_turn - 1) return -1;
...
static void update_previous_snapshot(const GameState *S) {
  ...
  for (int i = 0; i < S->warriors.len; ++i) {
    ...
    if (w->id.num >= 0 && w->id.num < DEFENSE_MAX_TRACKED_ID) {
      ...
      g_prev_turn[side_index(w->id.side)][w->id.num] = g_current_turn;
    }
  }
...
static int opponent_work_capacity(const GameState *S, const GameMap *M) {
  ...
  for (int i = 0; i < S->buildings.len; ++i) {
    const Building *b = &S->buildings.data[i];
    int c = (b->type == BTYPE_HQ) ? HQ_LEVELS[b->level].work_cap
                                  : BASE_LEVELS[b->level].work_cap;
    if (b->side == opp) cap += c; else mine += c;
  }
  /* NEXT VISION: we only know the enemy buildings we have seen.  The map is
     point-symmetric, so until every stronghold has been observed assume the
     enemy economy is at least as large as ours (mirror prior). */
  if (!all_strongholds_observed(S, M) && mine > cap) cap = mine;
  return cap;
}
...
static int issue_army_parity_training(Actions *a, const GameState *S,
  ...
  int theirs = count_side_warriors(S, opposite(M->my_side)); /* fog estimate */
...
static int hard_should_save_for_hq_now(const GameState *S, const GameMap *M,
  ...
  if (hq->level < HQ_MAX_LEVEL) {
    ...
    if (budget >= cost) return 1;
    if (net > 0 && turn <= MAX_TURN - 10 && budget + 8 * net >= cost) return 1;
  }
  if (hq->level == HQ_MAX_LEVEL && hq->hp < HQ_LEVELS[hq->level].hp) {
    ...
    if (net > 0 && turn <= MAX_TURN - 5 && budget + 6 * net >= HQ_HEAL_COST) return 1;
  }
  return 0;
}
...
static int collect_stage_candidates(const GameState *S, const GameMap *M,
  ...
  for (int i = 0; i < S->warriors.len && n < maxn; ++i) {
    ...
    if (turn >= MAX_TURN - 25) keep = min_int(1, remaining); /* late: finish the game */
    (void)keep;
    int already_taken = 0;
...
/* ===== NEXT VISION: scout ====================================================
   One warrior parks on a zone exactly VISION_HOPS hops from the enemy HQ that
   is not a stronghold and not on any engine path from the enemy HQ to a
   stronghold or to our HQ.  From there it sees the enemy HQ (level/hp), every
   warrior the enemy trains (exact trained count via ids) and the first hop of
   everything that leaves the HQ.  Turrets only hit their own zone, so the
   only threat is a warrior sent after it; an enemy adjacent to the post is a
   two-turn warning, and the scout kites to another candidate zone.
   Dispatch: once we have SCOUT_MIN_ARMY warriors; re-dispatched after death. */
#ifndef ENABLE_SCOUT
#define ENABLE_SCOUT 1
#endif
#ifndef SCOUT_MIN_ARMY
#define SCOUT_MIN_ARMY 4
#endif
#ifndef SCOUT_REDISPATCH_DELAY
#define SCOUT_REDISPATCH_DELAY 2
#endif
static int g_scout_num = -1;            /* warrior number of the live scout */
static int g_scout_post = -1;           /* zone it is assigned to */
static int g_scout_planned_target = -1; /* move issued by this module this turn */
static int g_scout_last_death_post = -1;
static int g_scout_next_dispatch_turn = 0;
static int g_scout_deaths = 0;
static IntVec g_scout_cands;            /* ranked candidate posts */
static int g_scout_cands_ready = 0;
static int *g_scout_hops_from_opp = NULL;
...
static void scout_prepare_candidates(const GameMap *M, const Paths *P) {
  if (g_scout_cands_ready) return;
  g_scout_cands_ready = 1;
  int N = M->N;
  g_scout_hops_from_opp = (int *)malloc((size_t)N * sizeof(int));
  bfs_hops_from(M, M->opp_hq, g_scout_hops_from_opp);
  int *hops_from_me = (int *)malloc((size_t)N * sizeof(int));
  bfs_hops_from(M, M->my_hq, hops_from_me);
  /* zones on the engine path from the enemy HQ to each objective */
  ...
  Cand *c = (Cand *)malloc((size_t)N * sizeof(Cand));
  int nc = 0;
  for (int pass = 0; pass < 2 && nc == 0; ++pass) {
    for (int r = 0; r < N; ++r) {
      if (g_scout_hops_from_opp[r] != VISION_HOPS) continue;
      if (is_stronghold(M, r) || r == M->my_hq || r == M->opp_hq) continue;
      if (pass == 0 && onpath[r]) continue;
      int bad_nb = 0;
      for (int k = 0; k < M->adj[r].len; ++k) if (onpath[M->adj[r].data[k]]) ++bad_nb;
      long long score = (long long)(pass == 0 ? 0 : 1000000) +
                        (long long)bad_nb * 10000 + hops_from_me[r] * 10 +
                        (long long)M->adj[r].len;
      c[nc].r = r; c[nc].score = score; ++nc;
    }
  }
  for (int i = 0; i < nc; ++i)          /* small n: insertion sort */
    for (int j = i; j > 0 && c[j].score < c[j - 1].score; --j) {
      Cand tmp = c[j]; c[j] = c[j - 1]; c[j - 1] = tmp;
    }
  for (int i = 0; i < nc; ++i) VEC_PUSH(g_scout_cands, c[i].r);
  free(c); free(onpath); free(hops_from_me);
#ifdef DEBUG_STATE
  fprintf(stderr, "SCOUT candidates:");
  for (int i = 0; i < g_scout_cands.len; ++i) fprintf(stderr, " %d", g_scout_cands.data[i]);
  fprintf(stderr, "\n");
#endif
}

static int scout_zone_threatened(const GameState *S, const GameMap *M, int zone) {
  Side opp = opposite(M->my_side);
  for (int i = 0; i < S->warriors.len; ++i) {
    const Warrior *w = &S->warriors.data[i];
    if (w->id.side != opp) continue;
    if (w->region == zone) return 1;
    for (int k = 0; k < M->adj[zone].len; ++k)
      if (M->adj[zone].data[k] == w->region) return 1;
  }
  return 0;
}
...
static int scout_pick_post(const GameState *S, const GameMap *M, int avoid) {
  for (int i = 0; i < g_scout_cands.len; ++i) {
    ...
    if (scout_zone_threatened(S, M, r)) continue;
    return r;
  }
  return -1;
}

static const Warrior *scout_find(const GameState *S, const GameMap *M) {
  if (g_scout_num < 0) return NULL;
  WarriorId id = {M->my_side, g_scout_num};
  return find_warrior_const(S, id);
}

static void scout_issue_move(Actions *a, WarriorId id, int target, int *budget) {
  Move mv = {id, target};
  VEC_PUSH(a->moves, mv);
  *budget -= MOVE_COST;
  g_scout_planned_target = target;
}

static void issue_scout(Actions *a, const GameState *S, const GameMap *M,
                        const Paths *P, int *budget, int turn, int hard_defense) {
  g_scout_planned_target = -1;
#if ENABLE_SCOUT
  scout_prepare_candidates(M, P);
  if (g_scout_cands.len == 0) return;

  const Warrior *sc = scout_find(S, M);
  if (g_scout_num >= 0 && sc == NULL) {          /* scout died: resend later */
    ++g_scout_deaths;
    g_scout_last_death_post = g_scout_post;
    g_scout_num = -1; g_scout_post = -1;
    g_scout_next_dispatch_turn = turn + SCOUT_REDISPATCH_DELAY;
#ifdef DEBUG_STATE
    fprintf(stderr, "SCOUT t=%d died (post %d), deaths=%d\n", turn, g_scout_last_death_post, g_scout_deaths);
#endif
  }

  if (sc == NULL) {
    if (hard_defense || turn < g_scout_next_dispatch_turn) return;
    if (count_side_warriors(S, M->my_side) < SCOUT_MIN_ARMY) return;
    if (*budget < MOVE_COST) return;
    int post = scout_pick_post(S, M, g_scout_last_death_post);
    if (post < 0) return;
    /* newest stationary warrior at the HQ that is not the last body there;
       fall back to any owned base with a spare stationary body */
    ...
    for (int pass = 0; pass < 2 && best == NULL; ++pass) {
      for (int i = 0; i < S->warriors.len; ++i) {
        ...
        const Building *b = find_building_const(S, w->region);
        if (b == NULL || b->side != M->my_side) continue;
        if (pass == 0 && w->region != M->my_hq) continue;
        int stay = 0;
        for (int j = 0; j < S->warriors.len; ++j) {
          const Warrior *o = &S->warriors.data[j];
          if (o->id.side == M->my_side && o->region == w->region &&
              o->state == WSTATE_STATIONARY && o->id.num != w->id.num &&
              !action_has_move_warrior(a, o->id)) ++stay;
        }
        if (stay < 1) continue;
    ...
    if (best == NULL) return;
    g_scout_num = best->id.num;
    g_scout_post = post;
    scout_issue_move(a, best->id, post, budget);
#ifdef DEBUG_STATE
    fprintf(stderr, "SCOUT t=%d dispatch %c%d -> %d (eta %d hops)\n", turn,
            side_char(best->id.side), best->id.num, post, path_hops_between(P, best->region, post));
#endif
    return;
  }

  if (sc->state != WSTATE_STATIONARY) return;   /* cannot redirect mid-move */
  int here = sc->region;
  if (scout_zone_threatened(S, M, here)) {
    /* kite: another candidate post, else any neighbour that is clear and not
       closer to the enemy HQ */
    if (*budget < MOVE_COST) return;
    ...
    if (to < 0 || to == here) {
      ...
      for (int k = 0; k < M->adj[here].len; ++k) {
        ...
        if (g_scout_hops_from_opp[v] < g_scout_hops_from_opp[here]) continue;
        if (to < 0 || g_scout_hops_from_opp[v] > g_scout_hops_from_opp[to]) to = v;
      }
    }
    if (to < 0) return;
    scout_issue_move(a, sc->id, to, budget);
#ifdef DEBUG_STATE
    fprintf(stderr, "SCOUT t=%d kite %d -> %d\n", turn, here, to);
#endif
    return;
  }
  if (here != g_scout_post) {
    /* back to (a clear) post */
    if (*budget < MOVE_COST) return;
    if (scout_zone_threatened(S, M, g_scout_post)) {
      int alt = scout_pick_post(S, M, g_scout_post);
      if (alt < 0) return;
      g_scout_post = alt;
    }
    scout_issue_move(a, sc->id, g_scout_post, budget);
  }
#else
  (void)a; (void)S; (void)M; (void)P; (void)budget; (void)turn; (void)hard_defense;
#endif
}

/* other modules must not borrow the scout: keep only the move this module
   planned (or none) */
static void scout_guard_moves(Actions *a, const GameState *S) {
  if (g_scout_num < 0) return;
  int wnew = 0;
  for (int i = 0; i < a->moves.len; ++i) {
    const Move *mv = &a->moves.data[i];
    if (mv->id.side == S->my_side && mv->id.num == g_scout_num &&
        mv->target != g_scout_planned_target)
      continue;
...
/* NOTE (NEXT VISION port): everything below still assumes it can see the whole
   enemy.  Under fog of war:
     - count_side_warriors(S, opp) / side_total_warrior_hp(S, opp) count only
       the enemy warriors visible RIGHT NOW (army parity, finisher, hard
       advantage ladder, force-training detection all read these);
     - enemy buildings are last-known memory, so "enemy bases cleared" means
       "none seen alive anywhere we have looked";
     - enemy_is_inbound_to_home()/attack_predict_update() only get a heading
       for enemies that were also visible on the previous turn.
   The gene values were tuned on 51..109-zone maps, a 500-gold/300-gold-base
   economy and 200 days; they need a re-tune, not just this port. */
static Actions decide(const GameState *S, const GameMap *M, const Paths *P,
  ...
  /* NEXT VISION: scout (below home defense, above every economy module) */
  issue_scout(&a, S, M, P, &budget, turn, hard_defense);
...
#endif
    {
      ...
      if (crr >= 0 && crr < M->N && find_building_const(S, crr) == NULL) {
        /* Race solvency floor (planner-only).  Always hold the wave retry
           ...
           the HQ is L2 (the hp-5 duel winner is coming) also hold the 300g
           center build: seed 1 won the duel at t64 but could only build at
           t93, losing the game.  Before HQ2 the 300 is NOT held -- it starves
...
static void moe_apply_expert_params(int N) {
  /* NEXT VISION: N is always 181..249, so the large-map expert (e = 3) is the
     one that fires.  The buckets are kept for -DFORCE_MOE_EXPERT experiments. */
#ifdef FORCE_MOE_EXPERT
  int e = FORCE_MOE_EXPERT; (void)N;
#else
  ...
  { static const int t[4] = {172, 172, 183, 172}; P_ENDGAME_SYNC_START_TURN = t[e] + LATE_TURN_SHIFT; }
  { static const int t[4] = {150, 190, 150, 150}; P_ENDGAME_SYNC_ARRIVAL_TURN = t[e] + LATE_TURN_SHIFT; }
  ...
  { static const int t[4] = {152, 152, 152, 152}; P_LATE_TIEBREAK_START_TURN = t[e] + LATE_TURN_SHIFT; }
  { static const int t[4] = {20, 20, 44, 28}; P_LATE_TIEBREAK_SAVE_LOOKAHEAD_TURNS = t[e]; }
  { static const int t[4] = {122, 116, 123, 122}; P_CASH_DUMP_TRAIN_START_TURN = t[e] + LATE_TURN_SHIFT; }
  { static const int t[4] = {899, 837, 1087, 899}; P_CASH_DUMP_MIN_GOLD = t[e]; }
  { static const int t[4] = {169, 169, 169, 174}; P_FINAL_HQ5_CASH_DUMP_START_TURN = t[e] + LATE_TURN_SHIFT; }
  { static const int t[4] = {195, 198, 195, 195}; P_FINAL_HQ5_HARD_DUMP_START_TURN = t[e] + LATE_TURN_SHIFT; }
  ...
  { static const int t[4] = {140, 140, 140, 140}; P_LATE_SAVER_START_TURN = t[e] + LATE_TURN_SHIFT; }
  ...
  { static const int t[4] = {150, 150, 150, 150}; P_FINISHER_START_TURN = t[e] + LATE_TURN_SHIFT; }
...
int main(void) {
  ...
  while (read_turn_start(&turn)) {
...
#ifdef DEBUG_STATE
    {
      ...
      fprintf(stderr, "STATE t=%d gold=%d mine=%d seen_enemy=%d my_bases=%d known_enemy_bases=%d vis=%d/%d enemy_trained>=%d\n",
              S.turn, S.gold, mine, seen_enemy, my_bases, known_enemy_bases, vis, M.N, enemy_trained_at_least(&S));
      fprintf(stderr, "INTEL t=%d est_alive=%d confirmed_dead=%d est_hq_lvl=%d est_hp=%d scout=%d post=%d\n",
              S.turn, enemy_alive_estimate(&S), g_ei_confirmed_dead, enemy_hq_level_estimate(&S, &M),
              enemy_total_hp_estimate(&S, &M), g_scout_num, g_scout_post);
      /* full dump for the referee cross-check (compare_truth.py) */
      fprintf(stderr, "BOT t=%d gold=%d vis=%d W=", S.turn, S.gold, vis);
      for (int i = 0, k = 0; i < S.warriors.len; ++i) {
        const Warrior *w = &S.warriors.data[i];
        if (w->id.side != M.my_side) continue;
        fprintf(stderr, "%s%c%d:%d:%d", k++ ? "," : "", side_char(w->id.side), w->id.num, w->region, w->hp);
      }
      fprintf(stderr, " B=");
      for (int i = 0, k = 0; i < S.buildings.len; ++i) {
        const Building *b = &S.buildings.data[i];
        if (b->side != M.my_side) continue;
        fprintf(stderr, "%s%d:%s:%d:%d", k++ ? "," : "", b->region, b->type == BTYPE_HQ ? "HQ" : "BASE", b->level, b->hp);
      }
      fprintf(stderr, " EB=");
      for (int i = 0, k = 0; i < S.buildings.len; ++i) {
        const Building *b = &S.buildings.data[i];
        if (b->side == M.my_side) continue;
        fprintf(stderr, "%s%d:%s:%d:%d:%s", k++ ? "," : "", b->region, b->type == BTYPE_HQ ? "HQ" : "BASE", b->level, b->hp,
                S.visible[b->region] ? "v" : "m");
      }
      fprintf(stderr, " EW=");
      for (int i = 0, k = 0; i < S.warriors.len; ++i) {
        const Warrior *w = &S.warriors.data[i];
