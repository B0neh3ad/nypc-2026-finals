...
#endif
#ifndef DEFENSE_FIRST_SIGHT_STACK_COUNT
#define DEFENSE_FIRST_SIGHT_STACK_COUNT 4
#endif
#ifndef DEFENSE_FIRST_SIGHT_RADIUS
#define DEFENSE_FIRST_SIGHT_RADIUS 6
#endif
#ifndef HOME_PRESSURE_MEMORY_MIN_STACK
#define HOME_PRESSURE_MEMORY_MIN_STACK 8
#endif
#ifndef HOME_PRESSURE_MEMORY_RADIUS
#define HOME_PRESSURE_MEMORY_RADIUS 6
#endif
#ifndef HOME_PRESSURE_HOLD_TURNS
#define HOME_PRESSURE_HOLD_TURNS 26
#endif
#ifndef HOME_PRESSURE_RESERVE_NUM
#define HOME_PRESSURE_RESERVE_NUM 4
#endif
#ifndef HOME_PRESSURE_RESERVE_DEN
#define HOME_PRESSURE_RESERVE_DEN 3
#endif
#ifndef HOME_PRESSURE_RESERVE_MARGIN
#define HOME_PRESSURE_RESERVE_MARGIN 2
#endif
#ifndef HOME_PRESSURE_RESERVE_MAX
#define HOME_PRESSURE_RESERVE_MAX 64
#endif
#ifndef HOME_PRESSURE_GARRISON_MARGIN
#define HOME_PRESSURE_GARRISON_MARGIN 2
#endif
#ifndef HOME_PRESSURE_GARRISON_MAX
#define HOME_PRESSURE_GARRISON_MAX 24
#endif
...
#define OPENING_NEUTRAL_FIRST_MAX_TURN 140
#endif
...
#define OPENING_RELAX_HQ_KEEP_UNTIL_TURN 80
#endif
...
#define OPENING_FORCE_TRAIN_NEUTRAL_UNTIL 80
#endif
...
#define THIN_OPENING_MAX_TURN 80
#endif
...
#endif
#ifndef POST_ATTACK_EXPANSION_GRACE_TURN
#define POST_ATTACK_EXPANSION_GRACE_TURN 100
#endif
...
static int g_home_pressure_until_turn = 0;
static int g_home_pressure_peak = 0;
...
static void parse_init(GameMap *M, GameState *S) {
  ...
  g_home_pressure_until_turn = 0;
  g_home_pressure_peak = 0;
...
static int post_attack_expansion_throttle_active(void) {
  return g_enemy_first_attack_seen &&
         g_current_turn > POST_ATTACK_EXPANSION_GRACE_TURN;
}
...
static int add_neutral_claim_move_with_build_reserve(Actions *a, const GameState *S,
  ...
  if (post_attack_expansion_throttle_active()) {
...
static int issue_neutral_builds(Actions *a, const GameState *S,
...
#if ENABLE_POST_ATTACK_NEUTRAL_WAIT_CONTROL
  if (post_attack_expansion_throttle_active() &&
      !post_attack_owned_work_slots_full(S, M, a))
    return 0;
  ...
  while (*budget >= BASE_LEVELS[1].cost) {
    ...
    *budget -= BASE_LEVELS[1].cost;
#if ENABLE_POST_ATTACK_NEUTRAL_WAIT_CONTROL
    if (post_attack_expansion_throttle_active()) ++post_attack_built;
#endif
    did = 1;
  }
  return did;
...
static int choose_anchor_route_return_anchor(const GameState *S,
  ...
  for (int bi = 0; bi < S->buildings.len; ++bi) {
    ...
    double d = P->dist[from_region][b->region];
...
static int issue_post_attack_neutral_waiter_control(Actions *a,
  ...
  return 0;
#else
  if (!post_attack_expansion_throttle_active()) return 0;
  int did = 0;
...
/* A clumped force first seen near the HQ is already urgent even when fog means
   no previous position exists. Keep memory of a serious wave so a withdrawal
   into fog does not immediately disperse the garrison before the follow-up. */
static int visible_home_pressure_count(const GameState *S, const GameMap *M,
                                       const Paths *P, int radius,
                                       int min_local_stack) {
  Side opp = opposite(M->my_side);
  int total = 0;
  int has_stack = 0;
  for (int i = 0; i < S->warriors.len; ++i) {
    ...
    ++total;
    if (enemy_warrior_count_at(S, M, w->region) >= min_local_stack)
      has_stack = 1;
  }
  return has_stack ? total : 0;
}

static void update_home_pressure_memory(const GameState *S, const GameMap *M,
                                        const Paths *P, int turn) {
  if (turn > g_home_pressure_until_turn) g_home_pressure_peak = 0;
  int visible = visible_home_pressure_count(
      S, M, P, HOME_PRESSURE_MEMORY_RADIUS, HOME_PRESSURE_MEMORY_MIN_STACK);
  if (visible < HOME_PRESSURE_MEMORY_MIN_STACK) return;
  if (visible > g_home_pressure_peak) g_home_pressure_peak = visible;
  g_home_pressure_until_turn = turn + HOME_PRESSURE_HOLD_TURNS;
}

static int home_pressure_reserve_target(int turn) {
  if (turn > g_home_pressure_until_turn || g_home_pressure_peak <= 0) return 0;
  int target = (g_home_pressure_peak * HOME_PRESSURE_RESERVE_NUM +
                HOME_PRESSURE_RESERVE_DEN - 1) /
                   HOME_PRESSURE_RESERVE_DEN +
               HOME_PRESSURE_RESERVE_MARGIN;
  return min_int(target, HOME_PRESSURE_RESERVE_MAX);
}

static int issue_home_pressure_reserve_training(Actions *a,
                                                const GameState *S,
                                                const GameMap *M,
                                                int *budget, int turn) {
  int target = home_pressure_reserve_target(turn);
  if (target <= 0) return 0;
  int mine = count_side_warriors(S, M->my_side);
  int need = target - (mine + a->train_n);
  int cap = planned_train_cap(S, M, a) - a->train_n;
  int n = min_int(need, cap);
  n = min_int(n, *budget / TRAIN_COST);
  while (n > 0) {
    int after_gold = *budget - TRAIN_COST * n;
    int income = side_current_income(S, M->my_side);
    int upkeep = UPKEEP_PER_WARRIOR * (mine + a->train_n + n);
    if (after_gold + 2 * (income - upkeep) >= 0) break;
    --n;
  }
  if (n <= 0) return 0;
  a->train_n += n;
...
static int enemy_is_inbound_to_home(const GameState *S, const GameMap *M,
  ...
  if (cur_h <= DEFENSE_FIRST_SIGHT_RADIUS &&
      enemy_warrior_count_at(S, M, w->region) >=
          DEFENSE_FIRST_SIGHT_STACK_COUNT)
    return 1;
...
static int issue_direct_hq_attack_guard(Actions *a, const GameState *S,
  ...
  int required_cnt = max_int(threat.count,
                             home_pressure_reserve_target(g_current_turn));
...
static Actions decide(const GameState *S, const GameMap *M, const Paths *P,
  ...
  update_home_pressure_memory(S, M, P, turn);
  if (turn <= g_home_pressure_until_turn && g_home_pressure_peak > 0) {
    int pressure_keep = min_int(
        g_home_pressure_peak + HOME_PRESSURE_GARRISON_MARGIN,
        HOME_PRESSURE_GARRISON_MAX);
    pressure_keep = min_int(pressure_keep,
                            count_side_warriors(S, M->my_side));
    g_home_defense_forced_hq_keep =
        max_int(g_home_defense_forced_hq_keep, pressure_keep);
  }
  ...
  issue_home_pressure_reserve_training(&a, S, M, &budget, turn);
