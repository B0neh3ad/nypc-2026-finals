/* NEW-RULE v3 (2026-08-29)
   v2 + three targeted fixes from log analysis:
   (1) opening anti-rush guard: one extra T1 train and a temporary 2-body HQ floor,
   (2) failed enemy-HQ wave memory: after a wave is confirmed dead, the next
       HQ wave must be at least max(previous+15, ceil(1.5*previous)),
   (3) rally-path blocker clearing: a synchronized HQ group may deliberately
       traverse a visible small enemy stack on the HQ->rally route instead of
       letting 1-2 blockers freeze the whole army.

   Port of v3_2(1).cpp to the current 400-day / fog-of-war rules.
   Scope: rule constants, wire protocol/state reconstruction, new map/ID safety,
   and mechanical 200->400 phase timing adaptation.  Core strategy ordering and
   non-rule hyperparameters are intentionally preserved rather than retuned.
   Enemy warriors and ordinary enemy buildings in GameState are the CURRENTLY
   VISIBLE snapshot only; no hidden enemy movement target/state is fabricated.

   v2 HQ-fog fix: the enemy HQ is a special exception.  Its region and continued
   existence are public game facts, so it is retained as a strategic attack
   target even outside vision.  Hidden level/HP are not treated as exact: the
   model below uses a conservative full-HP estimate and never fabricates hidden
   enemy warriors. */
...
/* Fog-of-war enemy-HQ strategic model.
   The enemy HQ location is public from the map and cannot move.  Keeping it as
   an attack target avoids the pathological behavior where HQ attacks become
   possible only after a scout happens to reveal the HQ.

   While the HQ is hidden, its combat parameters are deliberately conservative:
   - level >= last exactly observed level;
   - optionally level >= our current HQ level (+ margin);
   - HP is reset to the maximum HP of that assumed level, because a hidden HQ
     may have upgraded/repaired since the last observation.
   No hidden defenders are inserted; the existing HQ combat simulator already
   assumes full train-cap reinforcement under incomplete vision. */
...
/* Last exact enemy-HQ observation.  These are updated only from an authoritative
   BUILDING snapshot, never from our own synthetic hidden-HQ estimate. */
static int g_enemy_hq_exact_seen = 0;
static int g_enemy_hq_last_exact_level = 1;
static int g_enemy_hq_last_exact_hp = 10;
...
static void restore_strategic_enemy_hq(GameState *S, const GameMap *M) {
  ...
  if (visible_hq != NULL && visible_hq->side == enemy &&
    ...
    /* This function is called immediately after rebuilding from the authoritative
       snapshot, so an HQ already present here is an exact visible observation. */
    g_enemy_hq_exact_seen = 1;
    g_enemy_hq_last_exact_level = visible_hq->level;
    g_enemy_hq_last_exact_hp = visible_hq->hp;
    return;
  }
...
static void parse_init(GameMap *M, GameState *S) {
  ...
  g_enemy_hq_last_exact_hp = HQ_LEVELS[1].hp;
...
static void read_turn_result(GameState *S, const GameMap *M,
  ...
  /* New-rule protocol compatibility layer.
     Event sections (UPGRADE/TRAIN/MOVE/DAMAGE/SIEGE) contain only our events.
     WARRIOR/BUILDING are authoritative end-of-turn snapshots: all of our own
     objects plus only currently visible enemy objects.  The strategy layer is
     intentionally left unchanged and therefore sees only currently visible
     enemies, never stale hidden positions. */

  /* Apply our submitted morning costs/state locally first.  This preserves the
     old planner's exact own-state bookkeeping while the result block confirms
     the same own events. */
  ...
  {
    int n;
    char **t = tokens(readln(), &n); /* TIME self_ms self_tokens opp_ms opp_tokens */
  ...
  /* UPGRADE: own events only.  State was already applied above; consume rows. */
  {
    int n;
    char **t = tokens(readln(), &n);
    int count = atoi(t[1]);
  ...
  /* DAMAGE: HUNGER occurs after work/upkeep.  Delay only those HP losses until
     after local economic reconstruction, matching the official sample client. */
  ...
  /* Save the authoritative snapshots until after our local evening accounting.
     This matters because warriors killed by hunger still worked earlier that
     same evening. */
  ...
  /* Evening work: use pre-hunger own survivors. */
  int income = 0;
  ...
  /* Upkeep is paid in warrior-number order, but the number of successful 2g
     payments depends only on available gold.  HUNGER rows identify exactly
     which warriors took damage, so this reconstructs gold without guessing. */
  int alive = 0;
  for (int wi = 0; wi < S->warriors.len; ++wi)
  ...
  /* Rebuild the state from the authoritative snapshot.  Preserve moving target
     metadata only for our warriors; enemy movement intent is no longer part of
     the protocol and must not be fabricated. */
  ...
  /* Enemy ordinary buildings remain visibility-limited.  The HQ is special:
     its location/existence are public, so retain a conservative strategic model
     when it is outside our 2-hop vision.  This makes every existing HQ attack
     path (priority capture, rally stack, hard-stage finisher, path collision)
     see the HQ without teaching the bot any hidden unit information. */
...
/* v3 opening anti-rush guard.  Keep the successful two-pronged opening: on
   turn 1 two of the original three warriors may still leave, then TRAIN 1
   restores the HQ to two bodies before combat.  Through 6 + HQ-to-HQ hop
   distance, later modules may not plan the HQ below OPENING_ANTI_RUSH_HOME_KEEP. */
#ifndef ENABLE_OPENING_ANTI_RUSH_GUARD
#define ENABLE_OPENING_ANTI_RUSH_GUARD 1
#endif
#ifndef OPENING_ANTI_RUSH_EXTRA_TRAIN_TURN
#define OPENING_ANTI_RUSH_EXTRA_TRAIN_TURN 1
#endif
#ifndef OPENING_ANTI_RUSH_HOME_KEEP
#define OPENING_ANTI_RUSH_HOME_KEEP 2
#endif
#ifndef OPENING_ANTI_RUSH_BASE_TURNS
#define OPENING_ANTI_RUSH_BASE_TURNS 6
#endif
...
/* Center-second-base opening override.  The opponent logs often take the exact
   ...
   reserving the 500g build cost (so the normal first nearby base can still be
...
#define CENTER_SECOND_BASE_MAX_TURN 70
#endif
...
#endif
/* Bounded exit for the center race (fixes the never-ending mirror wave-loop).
   ECON_AFTER_TURN: while the race is still unresolved after this turn, the
   decide() gate stops ending the turn -- center claims are still issued (the
   center stays contested) but the normal economy/upgrade/attack modules run
   too, so the HQ no longer sits at L1 with 1,700 unspent gold.
   MAX_FAILED_WAVES: once this many dispatched center waves have died with the
   center still neutral, the dedicated race is abandoned for good (one-way
   latch); the ordinary neutral machinery may still claim the center later.
   0 disables the give-up. */
#ifndef CENTER_SPLIT_ECON_AFTER_TURN
static int P_CENTER_SPLIT_ECON_AFTER_TURN = 24;   /* MoE runtime param */
#define CENTER_SPLIT_ECON_AFTER_TURN P_CENTER_SPLIT_ECON_AFTER_TURN
#endif
#ifndef CENTER_SPLIT_MAX_FAILED_WAVES
static int P_CENTER_SPLIT_MAX_FAILED_WAVES = 25;   /* MoE runtime param */
#define CENTER_SPLIT_MAX_FAILED_WAVES P_CENTER_SPLIT_MAX_FAILED_WAVES
#endif
...
/* v3 HQ failed-wave state. */
static int g_hq_wave_inflight = 0;
static int g_hq_wave_inflight_size = 0;
static int g_hq_wave_launch_turn = -100000;
static int g_hq_wave_required_min = 0;
static int g_hq_wave_failures = 0;

static int hq_failed_wave_required_min(void) {
#if ENABLE_HQ_FAILED_WAVE_MEMORY
  return g_hq_wave_required_min;
#else
  return 0;
#endif
}
...
/* Number of distinct warriors dispatched toward the center by the split
   opening.  A new dispatch is only attempted after the previous wave is gone,
   so at dispatch time waves_sent equals the number of waves that already
   failed (died with the center still neutral). */
static int g_center_split_waves_sent = 0;
/* Economy-release standby keep: while this equals the current turn, the move
   choke point refuses to take the last stationary body out of the HQ for any
   non-center target -- that body is the next center wave. */
static int g_center_split_hold_hq_turn = -1;
static int g_center_split_center_region = -1;
/* Turn stamp of the last TRAIN issued by the center module (wave replacement
   or retry).  The thin-opening refund keeps one train on such a turn. */
...
#endif
/* ---- REPORT 11-8: the two movement pathologies -------------------------
   (a) ping-pong base rotation: a per-warrior order ledger (region left + turn)
       vetoed in the move choke point, so a warrior is never sent straight back
       to the region it was just ordered out of.  MOVE_FLAG_IGNORE_PINGPONG
       marks the moves that must always go through (defensive recalls, the
       HQ-upgrade worker pull, the retreats of (b)); deliberate combat moves
       (MOVE_FLAG_ALLOW_DANGER_TARGET) are exempt automatically.
   (b) pinned "retreat" orders: only order a retreat that can actually resolve
       (the engine refuses to move a warrior out of a region that contains an
       enemy warrior, and the order then pins it in MOVING state forever). */
#ifndef MOVE_FLAG_IGNORE_PINGPONG
#define MOVE_FLAG_IGNORE_PINGPONG 16
#endif
/* Deliberate group transit through a currently visible path blocker.  This is
   used only by the rally-blocker clearer; normal economy/staging movement keeps
   the full path safety veto. */
#ifndef MOVE_FLAG_IGNORE_PATH_ENEMY
#define MOVE_FLAG_IGNORE_PATH_ENEMY 32
#endif
#ifndef ENABLE_PINGPONG_ORDER_LEDGER
#define ENABLE_PINGPONG_ORDER_LEDGER 1
#endif
/* Veto window in turns; <= 0 disables the veto (the ledger is still kept, it
   costs two stores per issued move).  Measured on the 60-seed T1 gauntlet vs
   v3_base: window 15 cuts quick reversals 14.13 -> 1.63 per game and lifts
   warrior-days spent on an own building 78.0% -> 79.7%, but is score-neutral
   and margin-negative (dScore +0.003 [-0.012,+0.018], dMargin -0.29
   [-0.67,+0.11]) while the retreat fix alone is dScore +0.014 [+0.001,+0.035].
   So the gene ships at 0 and the GA may switch it on; 15 is the tested value. */
#ifndef PINGPONG_VETO_TURNS
static int P_PINGPONG_VETO_TURNS = 0;   /* MoE runtime param */
#define PINGPONG_VETO_TURNS P_PINGPONG_VETO_TURNS
#endif
...
/* Diagnostics for REPORT 11-8 (b): retreat orders that were actually issued
   and losing regions where every body was pinned by an enemy warrior. */
static int g_retreat_orders_issued = 0;
static int g_retreat_pinned_skipped = 0;
...
#endif
/* v2: in the large-map bucket, force the first HQ upgrade fund before normal
   parity/gather spending can repeatedly drain the account below 600 gold. */
#ifndef FORCE_HQ2_SAVE_TURN
static int P_FORCE_HQ2_SAVE_TURN = 0;   /* MoE runtime param */
#define FORCE_HQ2_SAVE_TURN P_FORCE_HQ2_SAVE_TURN
#endif
/* EVACFIX (r30 evacuation loss, battle 251179): minimum FREE enemy stack size that
   arms the mobile-stack launch guard / group pursuit. */
#ifndef EVAC_GUARD_MIN_STACK
static int P_EVAC_GUARD_MIN_STACK = 3;   /* MoE runtime param */
#define EVAC_GUARD_MIN_STACK P_EVAC_GUARD_MIN_STACK
#endif
...
/* no-trickle gate (REPORT 11-2): the live launchers only saw enemy stacks >= 2
   within 4 hops of the target; lone reinforcements, distant stacks and 1/turn
   streams were invisible, and a failed wave left no memory.  (a) travel-time
   reinforcements: any enemy that can reach the target within the wave's travel
   time (+ slack) joins the offensive-response sim as a scheduled defender.
   (b) re-attack memory: after a launch, remember the target's defense strength;
   while it is strictly stronger than at our last attempt, do not re-attack. */
#ifndef ENABLE_OFFENSE_REINF_TRAVEL
static int P_ENABLE_OFFENSE_REINF_TRAVEL = 1;   /* MoE runtime param */
#define ENABLE_OFFENSE_REINF_TRAVEL P_ENABLE_OFFENSE_REINF_TRAVEL
#endif
#ifndef OFFENSE_REINF_MIN_STACK
static int P_OFFENSE_REINF_MIN_STACK = 1;   /* MoE runtime param */
#define OFFENSE_REINF_MIN_STACK P_OFFENSE_REINF_MIN_STACK
#endif
#ifndef OFFENSE_REINF_ARRIVAL_SLACK
static int P_OFFENSE_REINF_ARRIVAL_SLACK = 2;   /* MoE runtime param */
#define OFFENSE_REINF_ARRIVAL_SLACK P_OFFENSE_REINF_ARRIVAL_SLACK
#endif
#ifndef ENABLE_REATTACK_MEMORY
static int P_ENABLE_REATTACK_MEMORY = 1;   /* MoE runtime param */
#define ENABLE_REATTACK_MEMORY P_ENABLE_REATTACK_MEMORY
#endif
#ifndef REATTACK_FORGET_TURNS
static int P_REATTACK_FORGET_TURNS = 30;   /* MoE runtime param */
#define REATTACK_FORGET_TURNS P_REATTACK_FORGET_TURNS
#endif

/* v3 enemy-HQ failed-wave memory.  Unlike ordinary BASE re-attack memory, this
   remembers wave SIZE, not the currently visible defense.  A wave is declared
   failed only after no own warrior remains at / moving toward the enemy HQ. */
#ifndef ENABLE_HQ_FAILED_WAVE_MEMORY
#define ENABLE_HQ_FAILED_WAVE_MEMORY 1
#endif
#ifndef HQ_FAILED_WAVE_MIN_RECORD
#define HQ_FAILED_WAVE_MIN_RECORD 8
#endif
#ifndef HQ_FAILED_WAVE_ADD
#define HQ_FAILED_WAVE_ADD 15
#endif
#ifndef HQ_FAILED_WAVE_RATIO_NUM
#define HQ_FAILED_WAVE_RATIO_NUM 3
#endif
#ifndef HQ_FAILED_WAVE_RATIO_DEN
#define HQ_FAILED_WAVE_RATIO_DEN 2
#endif
#ifndef HQ_FAILED_WAVE_MAX_REQUIRED
#define HQ_FAILED_WAVE_MAX_REQUIRED 180
#endif

/* v3 rally-path blocker clearing.  Only small, currently visible warrior stacks
   on the HQ->owned-rally shortest path are eligible.  The clearing group keeps
   target=rally, so after winning the intermediate fight it automatically
   continues to the rally under the persistent-MOVE rule. */
#ifndef ENABLE_RALLY_PATH_BLOCKER_CLEAR
#define ENABLE_RALLY_PATH_BLOCKER_CLEAR 1
#endif
#ifndef RALLY_BLOCKER_CLEAR_MAX_ENEMY
#define RALLY_BLOCKER_CLEAR_MAX_ENEMY 8
#endif
#ifndef RALLY_BLOCKER_CLEAR_MIN_WAVE
#define RALLY_BLOCKER_CLEAR_MIN_WAVE 4
#endif
#ifndef RALLY_BLOCKER_CLEAR_EXTRA_MARGIN
#define RALLY_BLOCKER_CLEAR_EXTRA_MARGIN 2
#endif
#ifndef RALLY_BLOCKER_CLEAR_MAX_WAVE
#define RALLY_BLOCKER_CLEAR_MAX_WAVE 30
#endif
...
/* ---- SAVER (REPORT 11-5): threat-conditioned late HQ saver ---------------
   From LATE_SAVER_START_TURN, the same "bank the next HQ level" reservation the
   t152 tiebreak already does, but (a) earlier and (b) released as soon as
   something is actually close to our own HQ, where bodies now beat one more HQ
   level later.  10 of the r24-30 draws were static-upper-bound losable by one
   HQ level that the cash dump had already trained away. */
#ifndef LATE_SAVER_START_TURN
static int P_LATE_SAVER_START_TURN = 140;   /* MoE runtime param */
#define LATE_SAVER_START_TURN P_LATE_SAVER_START_TURN
#endif
#ifndef LATE_SAVER_LOOKAHEAD_TURNS
static int P_LATE_SAVER_LOOKAHEAD_TURNS = 12;   /* MoE runtime param */
#define LATE_SAVER_LOOKAHEAD_TURNS P_LATE_SAVER_LOOKAHEAD_TURNS
#endif
#ifndef LATE_SAVER_THREAT_RADIUS
static int P_LATE_SAVER_THREAT_RADIUS = 2;   /* MoE runtime param */
#define LATE_SAVER_THREAT_RADIUS P_LATE_SAVER_THREAT_RADIUS
#endif
#ifndef LATE_SAVER_THREAT_MIN
static int P_LATE_SAVER_THREAT_MIN = 1;   /* MoE runtime param */
#define LATE_SAVER_THREAT_MIN P_LATE_SAVER_THREAT_MIN
#endif
#ifndef LATE_SAVER_RELEASE_ON_THREAT
static int P_LATE_SAVER_RELEASE_ON_THREAT = 1;   /* MoE runtime param */
#define LATE_SAVER_RELEASE_ON_THREAT P_LATE_SAVER_RELEASE_ON_THREAT
#endif
#ifndef LATE_SAVER_GATHER_RADIUS
static int P_LATE_SAVER_GATHER_RADIUS = 6;   /* MoE runtime param */
#define LATE_SAVER_GATHER_RADIUS P_LATE_SAVER_GATHER_RADIUS
#endif
#ifndef LATE_SAVER_LOCK
static int P_LATE_SAVER_LOCK = 1;   /* MoE runtime param */
#define LATE_SAVER_LOCK P_LATE_SAVER_LOCK
#endif

/* ---- FINISHER (REPORT 11-6): turn the idle late army into HQ-hp margin ----
   76/79 of the lineage's turn-limit wins ended at margin = 5 x HQ level gap
   with the spare army standing still.  The binding constraint is the enemy's
   whole field army coming home, so the gate is a RATIO test on their army plus
   "nothing near our own HQ", and it opens only after the HQ fund above is
   settled. */
#ifndef FINISHER_START_TURN
static int P_FINISHER_START_TURN = 150;   /* MoE runtime param */
#define FINISHER_START_TURN P_FINISHER_START_TURN
#endif
#ifndef FINISHER_MIN_ARMY
static int P_FINISHER_MIN_ARMY = 8;   /* MoE runtime param */
#define FINISHER_MIN_ARMY P_FINISHER_MIN_ARMY
#endif
#ifndef FINISHER_ARMY_RATIO_NUM
static int P_FINISHER_ARMY_RATIO_NUM = 2;   /* MoE runtime param */
#define FINISHER_ARMY_RATIO_NUM P_FINISHER_ARMY_RATIO_NUM
#endif
#ifndef FINISHER_ARMY_RATIO_DEN
static int P_FINISHER_ARMY_RATIO_DEN = 1;   /* MoE runtime param */
#define FINISHER_ARMY_RATIO_DEN P_FINISHER_ARMY_RATIO_DEN
#endif
#ifndef FINISHER_MAX_ENEMY_INCOME
static int P_FINISHER_MAX_ENEMY_INCOME = 150;   /* MoE runtime param */
#define FINISHER_MAX_ENEMY_INCOME P_FINISHER_MAX_ENEMY_INCOME
#endif
#ifndef FINISHER_HOME_SAFE_RADIUS
static int P_FINISHER_HOME_SAFE_RADIUS = 4;   /* MoE runtime param */
#define FINISHER_HOME_SAFE_RADIUS P_FINISHER_HOME_SAFE_RADIUS
#endif
#ifndef FINISHER_CAP_TRAIN
static int P_FINISHER_CAP_TRAIN = 1;   /* MoE runtime param */
#define FINISHER_CAP_TRAIN P_FINISHER_CAP_TRAIN
#endif
#ifndef FINISHER_HQ_TARGET_RELAX
static int P_FINISHER_HQ_TARGET_RELAX = 1;   /* MoE runtime param */
#define FINISHER_HQ_TARGET_RELAX P_FINISHER_HQ_TARGET_RELAX
#endif
#ifndef FINISHER_STREAM
static int P_FINISHER_STREAM = 1;   /* MoE runtime param */
#define FINISHER_STREAM P_FINISHER_STREAM
#endif
#ifndef FINISHER_ATTACK_WAIT_FOR_HQ
static int P_FINISHER_ATTACK_WAIT_FOR_HQ = 1;   /* MoE runtime param */
#define FINISHER_ATTACK_WAIT_FOR_HQ P_FINISHER_ATTACK_WAIT_FOR_HQ
#endif
#ifndef FINISHER_KILL_SLACK_TURNS
static int P_FINISHER_KILL_SLACK_TURNS = 15;   /* MoE runtime param */
#define FINISHER_KILL_SLACK_TURNS P_FINISHER_KILL_SLACK_TURNS
#endif
...
/* v21-stack-cashdump: once HQ5 is secured, the final resource objective is
   ...
   warriors, especially in turns 360..400 where unused gold no longer compounds. */
...
/* Enemy MOVE targets are not exposed by the new-rule protocol.  Legacy exact
   next-hop/committed-target helpers were removed; projected stack logic below
   deliberately uses only the currently visible enemy stack. */

static int enemy_projected_stack_count_at(const GameState *S,
                                          const GameMap *M,
                                          const Paths *P, int region) {
  /* New-rule fog protocol never exposes an enemy warrior's movement target.
     Enemy snapshots therefore cannot support the old exact next-hop/committed
     projection.  Treat the current visible stack as the guaranteed lower
     bound; directional threat inference is handled separately by the
     consecutive-observation attack predictor. */
  (void)P;
...
/* Per-warrior order ledger for the ping-pong veto (REPORT 11-8 / architecture
   hook (g)).  g_pp_from[id] is the region the warrior was last ordered OUT of
   and g_pp_turn[id] the turn that order was issued.  Only our own warriors are
   ever ordered, so a single side-agnostic table is enough. */
static int g_pp_init = 0;
static int g_pp_from[DEFENSE_MAX_TRACKED_ID];
static int g_pp_turn[DEFENSE_MAX_TRACKED_ID];
static int g_pp_vetoes = 0;          /* diagnostics only */

static void pingpong_ledger_init(void) {
  if (g_pp_init) return;
#ifdef DEBUG_PATCH
  setvbuf(stderr, NULL, _IONBF, 0);   /* the engine loses block-buffered stderr */
#endif
...
/* True when `target` is the region this warrior was ordered out of no more
   than PINGPONG_VETO_TURNS turns ago: the second leg of a rotation loop. */
static int pingpong_move_is_reversal(const GameMap *M, WarriorId id, int target,
  ...
  if ((flags & (MOVE_FLAG_IGNORE_PINGPONG | MOVE_FLAG_ALLOW_DANGER_TARGET)) != 0)
    return 0;
  /* Moves home are never vetoed: the HQ is the destination of every defensive
     recall and of the HQ-upgrade worker pull.  The outbound leg of an
     HQ<->base loop is still cut, so the loop is broken exactly once. */
...
static int add_move_action_ex_stack_flags(Actions *a, const GameState *S,
  ...
  /* Ping-pong veto (REPORT 11-8): refuse to send this warrior straight back to
     the region it was ordered out of within the last PINGPONG_VETO_TURNS
     turns.  Defensive/urgent orders carry MOVE_FLAG_IGNORE_PINGPONG (or
     MOVE_FLAG_ALLOW_DANGER_TARGET) and are never vetoed, so only the economic
     return leg of a rotation loop is cut. */
  if (pingpong_move_is_reversal(M, id, target, flags)) {
    ++g_pp_vetoes;
#ifdef DEBUG_PATCH
    fprintf(stderr, "pp-veto t=%d id=%d back_to=%d left_at_t=%d\n",
            g_current_turn, id.num, target, g_pp_turn[id.num]);
#endif
    return 0;
  }
  ...
  int allow_enemy_on_target = target_enemy_building || target == M->my_hq ||
      ((flags & MOVE_FLAG_ALLOW_DANGER_TARGET) != 0);
  if ((flags & MOVE_FLAG_IGNORE_PATH_ENEMY) == 0 &&
  ...
  /* Center-race standby keep (economy-release mode): while the center race is
     still on and the normal economy modules are running, one stationary HQ
     body stays reserved as the next center wave.  Without it the dispatch
     modules drain the HQ, the post-KO retry needs a TRAIN turn, and a
     frozen-economy opponent wins the center race by exactly that tempo. */
...
static int center_second_base_opening_active(const GameState *S,
  ...
  if (CENTER_SPLIT_MAX_FAILED_WAVES > 0 &&
    ...
    /* Bounded exit: every dispatched center wave has died with the center
       still neutral.  Stop feeding one-body waves forever; the ordinary
       neutral machinery may still claim/build the center later. */
#ifdef DEBUG_PATCH
    fprintf(stderr, "center-giveup turn=%d waves=%d\n", turn,
...
static int center_second_base_source_eta_ex(const GameState *S, const GameMap *M,
  ...
  for (int wi = 0; wi < S->warriors.len; ++wi) {
    ...
    /* At equal ETA prefer the highest-HP body: after a mid-race HQ upgrade the
       hp-5 trainee, not a leftover hp-4 worker, should fight the next duel. */
...
#ifdef DEBUG_PATCH
  fprintf(stderr, "refill t=%d rem=%d budget=%d cap=%d train_n=%d\n",
          g_current_turn, hq_remaining, *budget, planned_train_cap(S, M, a),
          a->train_n);
#endif
...
static int issue_center_second_base_claim(Actions *a, const GameState *S,
  ...
  for (int k = 0; k < CENTER_SECOND_BASE_MAX_SEND_PER_TURN && sent < 1; ++k) {
    ...
    if (add_move_action(a, S, M, id, center, budget)) {
      ...
      ++g_center_split_waves_sent;
#ifdef DEBUG_PATCH
      fprintf(stderr, "center-wave turn=%d n=%d\n", g_current_turn,
              g_center_split_waves_sent);
#endif
...
/* New-rule fog-of-war visibility helpers.  Under the current protocol,
   absence from S->buildings / S->warriors means "not currently visible", not
   "does not exist".  Any strategy gate that needs a global negative fact must
   first prove that the relevant map is fully observed. */
...
static int enemy_normal_bases_cleared_now(const GameState *S, const GameMap *M) {
  /* Fog-of-war: zero VISIBLE enemy bases is not evidence that all enemy bases
     are gone.  This optimization is legal only when every stronghold is under
     our current vision, so the negative observation is actually global. */
...
static int retreat_sim_losing_region(const GameState *S, const GameMap *M,
  ...
  if (b != NULL && b->side != M->my_side) {
    ...
    if (b->type == BTYPE_HQ) {
      /* Hidden enemy bases make visible income only a lower bound.  Unless the
         whole map is currently visible, assume the HQ can fill its train cap
         each simulated day instead of declaring a retreat safe from incomplete
         economy information. */
...
static int issue_losing_fight_retreats(Actions *a, const GameState *S, const GameMap *M, const Paths *P, int *budget) {
  ...
  for (int r=0; r<M->N && r<256; ++r) {
    ...
    /* REPORT 11-8 (pinned retreats).  The engine only advances a moving
       warrior when its CURRENT region holds no enemy warrior (testing-tool
       apply_day_movement).  A "retreat" ordered out of a contested region can
       therefore never resolve: the warrior fights where it stands anyway, but
       the order latches it into MOVING state, where no module may re-order it
       and where it is invisible to every stationary-warrior count -- 7.8
       deaths per game, 23% of ours, with the last order issued from the death
       region.  Only order the retreat when it can actually resolve (no enemy
       warrior in r right now: movement happens before combat, so the warrior
       also escapes the turret of an enemy building there).  When the region is
       already contested we stand and fight instead; g_retreat_bad_region[r]
       still stops us from feeding more bodies in. */
    if (pinned_by_enemy) {
      ++g_retreat_pinned_skipped;
#ifdef DEBUG_PATCH
      pingpong_ledger_init();
      fprintf(stderr, "retreat-pinned-skip t=%d region=%d mine=%d\n",
              g_current_turn, r, count_stationary_warriors_at(S,M->my_side,r));
#endif
      continue;
    }
#endif
    for (int wi=0; wi<S->warriors.len; ++wi) {
      ...
      if (add_move_action_ex_stack_flags(a,S,M,P,w->id,safe,budget,
        ...
        did=1;
        ++g_retreat_orders_issued;
#ifdef DEBUG_PATCH
        fprintf(stderr, "retreat-order t=%d id=%d from=%d to=%d pinned=%d\n",
                g_current_turn, w->id.num, r, safe, pinned_by_enemy);
#endif
      }
    }
  }
...
/* SAVER: how many enemy warriors are within `radius` hops of our own HQ (a
   warrior already committed to moving onto the HQ always counts).  Uses the
   per-turn Paths global set at the top of decide(), so none of the save/train
   predicates below need a signature change. */
...
static int late_hq_tiebreak_should_save(const GameState *S, const GameMap *M,
  ...
  if (action_has_upgrade(a, M->my_hq)) return 0;
  /* SAVER (REPORT 11-5): release the HQ fund while something is actually on top
     of our HQ - bodies now beat one more HQ level later. */
  if (LATE_SAVER_RELEASE_ON_THREAT &&
      late_saver_hq_threat_now(S, M, LATE_SAVER_THREAT_RADIUS) >=
          LATE_SAVER_THREAT_MIN) {
#ifdef DEBUG_PATCH
    fprintf(stderr, "# Debug SAVER release turn %d\n", turn);
#endif
    return 0;
  }
...
/* SAVER (REPORT 11-5): the early half of the late saver.  Between
   LATE_SAVER_START_TURN and the base LATE_TIEBREAK_START_TURN, reserve the next
   HQ level as soon as gold + projected net income covers it, unless an enemy is
   within LATE_SAVER_THREAT_RADIUS hops of our HQ. */
static int late_saver_active(const GameState *S, const GameMap *M,
  ...
  if (turn >= LATE_TIEBREAK_START_TURN) return 0;   /* base machinery owns it */
  if (action_has_upgrade(a, M->my_hq)) return 0;
  ...
  int on = 0;
  if (budget >= cost) {
    /* Affordable: only hold the gold while this morning is not legal yet (a
       worker is still walking home); otherwise it is upgraded this turn. */
...
#ifdef DEBUG_PATCH
  if (on)
    fprintf(stderr, "# Debug SAVER hold turn %d gold %d cost %d\n", turn,
            budget, cost);
#endif
  return on;
}
...
static int issue_forced_hq2_save_or_upgrade(Actions *a, const GameState *S,
  ...
  force_hq2_upgrade_worker(a, S, M, P, budget);
...
static int issue_underfilled_building_worker_support(Actions *a,
  ...
  if (!hq_save_lock) {
    ...
    /* Do not spend the base-upgrade reserve merely to fill HQ-only work slots.
       ...
       many legal BASE 1->2 upgrades were still waiting for the 550-gold budget.
...
/* ==== EVACFIX (r30 evacuation loss, battle 251179; see analysis/findings/next/evacuation-r30.md)
   F1: before committing an unrecallable capture wave at a FREE enemy stack, check whether the
   stack could decline the fight and reach one of our under-garrisoned buildings within the
   wave's own ETA (moves are locked until arrival, so the wave cannot come back in time). ==== */
#ifndef EVACFIX_F1
#define EVACFIX_F1 1
#endif
#ifndef EVACFIX_F2
...
/* Would the remaining garrison of our building at breg (units not already ordered away this
   turn) + its turret hold against the given enemy stack walking in?  Defender-side mirror of
   combat_simulation_win's day loop; conservative: no training, no reinforcements, a stalemate
   counts as holding only while the building stands. */
...
#if EVACFIX_F2
/* Threatened-building group rally (the evacuation counter-move).  MOVE targets must be
   strongholds/HQs and an equal-speed follower always lands one hop behind, so chasing the
   stack's CURRENT region is useless (251179: the evacuees were on transit region 86 when we
   could first react).  Instead: when an enemy mass (>= EVAC_GUARD_MIN_STACK, transit regions
   counted) stands within EVACFIX_STACK_HOPS of one of our buildings whose garrison would NOT
   hold, and >= 2 idle non-worker units stand nearby, send the WHOLE group onto the building in
   one order (bypassing full_path_enemy_blocked - meeting the stack en route as a group is the
   point), gated by an ETA-aware sim in which late arrivals join the running fight. */
...
static int issue_evacuated_stack_pursuit(Actions *a, const GameState *S,
  ...
  Side opp = opposite(M->my_side);
  int did = 0;
  (void)budget; /* rally onto our own building: the move is free */
  for (int bi = 0; bi < S->buildings.len; ++bi) {
    const Building *b = &S->buildings.data[bi];
    if (b->side != M->my_side) continue;
    int breg = b->region;
    ...
    for (int i = 0; i < S->warriors.len && gn < MAX_COMBAT_SIM_UNITS; ++i) {
      ...
      if (action_has_move_warrior(a, w->id)) continue;
      const Building *sb = find_building_const(S, w->region);
      if (sb != NULL && sb->side == M->my_side) continue;  /* keep workers/garrisons */
      int h = path_hops_between(P, w->region, breg);
    ...
    if (gn < 2) continue;
    /* ETA-aware defense sim at breg: whole enemy stack arrives day 0 (pessimistic),
       garrison+turret fight from day 0, group members join at their lag. */
    {
    ...
    /* Deliberate direct push: add_move_action_ex_stack_flags would refuse whenever the enemy
       stack sits on an intermediate hop (full_path_enemy_blocked has no override flag), but
       meeting the stack en route with the whole group is exactly the intent.  Constraints
       already checked: own stationary unplanned warrior, off-building source, breg is our
       building (legal destination, move is free). */
    int sent = 0;
    for (int i = 0; i < gn; ++i) {
      Move mv;
      mv.id = ids[i];
...
#ifdef DEBUG_PATCH
    fprintf(stderr, "# Debug evacfix F2 pursuit: building %d threatened by %d-stack (eta %d), rallied %d units\n",
            breg, stack_n, eta_stack, sent);
#endif
    if (sent >= 2) did = 1;
  }
  return did;
...
static int combat_simulation_win(const GameState *S, const GameMap *M,
  ...
  if (b->type == BTYPE_HQ && target == M->opp_hq &&
      attacker_n < hq_failed_wave_required_min())
    return 0;
  ...
  if (b->type == BTYPE_HQ) {
    ...
    /* Under fog, hidden bases can fund HQ training even when visible enemy
       income is zero.  Use exact income only with complete map vision;
       otherwise conservatively allow the enemy to fill the HQ train cap. */
    if (all_regions_visible_to_me(S, M)) {
...
#if EVACFIX_F1
  {
    /* EVACFIX F1: the target stack is FREE (enemy-occupied neutral stronghold, nothing pins
       it).  If it can decline the fight and reach one of our under-garrisoned buildings
       before this wave could return, do not launch (251179 T91). */
    int stack_hp[MAX_COMBAT_SIM_UNITS];
    int stack_n = 0;
...
#ifdef DEBUG_PATCH
          fprintf(stderr, "# Debug evacfix F1 refuse: wave need=%d eta=%d at free %d-stack on %d; building %d exposed (h=%d)\n",
                  need, wave_eta, stack_n, target, b->region, h);
#endif
          free(pool);
          return 0;
        }
...
/* no-trickle re-attack memory (REPORT 11-2): remember the target's defense
   strength (defender HP + building HP) at every wave launch.  While the target
   is strictly stronger than at our last attempt, do not attack it again --
   reinforced bases fell 1/75 in the recorded games.  Enemy HQ is exempt (the
   HQ sim already models its training and blocking the win target could stall
   the endgame).  Memory expires after REATTACK_FORGET_TURNS. */
...
#ifdef DEBUG_PATCH
  fprintf(stderr, "# DBG-TRICKLE t%d mem-record target=%d def=%d\n",
          g_current_turn, target, g_attack_mem_def[target]);
#endif
}
...
#ifdef DEBUG_PATCH
    fprintf(stderr, "# DBG-TRICKLE t%d mem-block target=%d def=%d was=%d\n",
            g_current_turn, target, attack_mem_strength(S, M, b),
            g_attack_mem_def[target]);
#endif
    return 1;
  }
  return 0;
...
/* v3: synchronized transit group for a visible small blocker on the HQ->rally path. */
static int first_rally_warrior_blocker(const GameState *S, const GameMap *M,
                                       const Paths *P, int from, int rally) {
  if (P == NULL || from < 0 || rally < 0 || from >= P->N || rally >= P->N)
    return -1;
  if (P->nxt[from][rally] == -1) return -1;
  int cur = from, guard = 0;
  while (cur != rally && guard++ <= P->N + 5) {
    cur = P->nxt[cur][rally];
    if (cur < 0 || cur == rally) break;
    const Building *b = find_building_const(S, cur);
    if (b != NULL && b->side != M->my_side) return -1;
    if (enemy_warrior_count_at(S, M, cur) > 0) return cur;
  }
  return -1;
}

static int issue_rally_path_blocker_clear_from_hq(Actions *a,
                                                   const GameState *S,
                                                   const GameMap *M,
                                                   const Paths *P,
                                                   int rally,
                                                   int *budget) {
#if ENABLE_RALLY_PATH_BLOCKER_CLEAR
  if (rally == M->my_hq) return 0;
  const Building *rb = find_building_const(S, rally);
  if (rb == NULL || rb->side != M->my_side) return 0;
  if (region_has_enemy_warrior(S, M, M->my_hq)) return 0;

  int blocker = first_rally_warrior_blocker(S, M, P, M->my_hq, rally);
  if (blocker < 0) return 0;
  int en = enemy_warrior_count_at(S, M, blocker);
  if (en <= 0 || en > RALLY_BLOCKER_CLEAR_MAX_ENEMY) return 0;

  int keep = planned_work_cap_at(S, M, a, M->my_hq);
  if (g_home_defense_forced_hq_keep > keep)
    keep = g_home_defense_forced_hq_keep;
#if ENABLE_OPENING_ANTI_RUSH_GUARD
  {
    int hh = path_hops_between(P, M->my_hq, M->opp_hq);
    int end_turn = (hh >= INF_HOPS) ? OPENING_ANTI_RUSH_BASE_TURNS
                                    : OPENING_ANTI_RUSH_BASE_TURNS + hh;
    if (g_current_turn <= end_turn)
      keep = max_int(keep, OPENING_ANTI_RUSH_HOME_KEEP);
  }
#endif

  int remaining = planned_workers_physically_remaining_at(S, a, M->my_side, M->my_hq);
  int avail = remaining - keep;
  if (avail < RALLY_BLOCKER_CLEAR_MIN_WAVE) return 0;
  if (avail > RALLY_BLOCKER_CLEAR_MAX_WAVE) avail = RALLY_BLOCKER_CLEAR_MAX_WAVE;

  int hp[MAX_COMBAT_SIM_UNITS];
  WarriorId ids[MAX_COMBAT_SIM_UNITS];
  int n = 0;
  for (int i = S->warriors.len - 1; i >= 0 && n < avail; --i) {
    const Warrior *w = &S->warriors.data[i];
  ...
  if (n < RALLY_BLOCKER_CLEAR_MIN_WAVE) return 0;

  int minwin = 0;
  for (int k = 1; k <= n; ++k) {
    if (neutral_combat_simulation_win(S, M, blocker, hp, k)) {
      minwin = k;
      break;
    }
  }
  if (minwin <= 0) return 0;
  int need = max_int(RALLY_BLOCKER_CLEAR_MIN_WAVE,
                     minwin + RALLY_BLOCKER_CLEAR_EXTRA_MARGIN);
  if (need > n || need > RALLY_BLOCKER_CLEAR_MAX_WAVE) return 0;

  int sent = 0;
  for (int i = 0; i < need; ++i) {
    int flags = MOVE_FLAG_IGNORE_PATH_ENEMY | MOVE_FLAG_IGNORE_STACK_GUARD |
                MOVE_FLAG_IGNORE_PINGPONG;
    if (add_move_action_ex_stack_flags(a, S, M, P, ids[i], rally, budget, flags))
      ++sent;
  }
#ifdef DEBUG_PATCH
  if (sent > 0)
    fprintf(stderr, "# DBG-BLOCKER t%d rally=%d blocker=%d enemy=%d sent=%d minwin=%d\n",
            g_current_turn, rally, blocker, en, sent, minwin);
#endif
  return sent;
#else
  (void)a; (void)S; (void)M; (void)P; (void)rally; (void)budget;
  return 0;
#endif
}
...
static int issue_rally_stack_staging(Actions *a, const GameState *S,
  ...
  {
    int cleared = issue_rally_path_blocker_clear_from_hq(a, S, M, P, rally, budget);
...
static int hq_surplus_anchor_desired_at_rally(const GameState *S,
  ...
  } else if (tb != NULL && tb->type == BTYPE_HQ && target == M->opp_hq) {
    desired = max_int(desired, hq_failed_wave_required_min());
    desired = min_int(desired, MAX_COMBAT_SIM_UNITS);
  }
...
static int unclaimed_empty_neutral_exists(const GameState *S, const GameMap *M,
  ...
  for (int i = 0; i < M->strongholds.len; ++i) {
    ...
    /* A hidden stronghold is UNKNOWN, not confirmed empty.  Unknown objectives
       may still be scouted by the expansion code, but they must not globally
       block rally/attack logic as if we knew they were free neutrals. */
...
static int opening_neutral_target_goal(const GameMap *M) {
  ...
  /* New maps are centrally symmetric and fogged.  Requiring the old 82% of
     ALL strongholds before leaving the opening assumes global information and
     asks us to own deep enemy-side objectives before normal combat may begin.
     The natural uncontested opening share is our half plus the center. */
...
static int neutral_build_ready_on_arrival_after_train_delay(
  ...
  for (int day = 0; day <= build_day; ++day) {
    /* Other warriors that are already sitting on or moving toward empty
       strongholds will consume 500 gold before this newly trained worker's
...
static int issue_immediate_hq_neutral_dispatch(Actions *a, const GameState *S,
  ...
  /* Newly trained neutral workers should not spend a full turn idling at HQ.
     ...
     immediately, even if the 500-gold build will be paid on arrival rather
     than reserved today. */
...
static void update_previous_snapshot(const GameState *S) {
  strategy_prev_init();
  /* New-rule fog of war: previous_region is meaningful only for units that
     were present in the immediately preceding snapshot.  Clear absent IDs so
     a unit that disappears into fog and reappears later is not mistaken for a
     one-turn long-distance move. */
  for (int s = 0; s < 2; ++s)
    for (int i = 0; i < DEFENSE_MAX_TRACKED_ID; ++i)
      g_prev_region[s][i] = -1;
  for (int i = 0; i < S->warriors.len; ++i) {
...
static int enemy_is_inbound_to_home(const GameState *S, const GameMap *M,
  ...
  /* Enemy MOVE destinations are hidden in the new protocol.  Infer inbound
     motion only from consecutive visible positions. */
...
static DirectHqThreat direct_hq_threat_stats(const GameState *S,
  ...
  for (int i = 0; i < S->warriors.len; ++i) {
    ...
    if (w->region == M->my_hq) {
      ...
      /* No hidden target is available: use observable proximity / consecutive
         approach instead of the legacy enemy moving_target test. */
      hq_bound = 1;
    }
...
static int enemy_eta_to_owned_base(const GameState *S, const GameMap *M,
  ...
  /* Enemy destination is hidden.  Exact ETA is known only after contact;
     nearby/predicted ETAs are added by collect_enemy_base_attack_schedule(). */
...
static int g_dbg_travel_reinf_n = 0;   /* no-trickle: travel-rule-only defenders in the last collect */
...
static int collect_offensive_response_defenders(const GameState *S,
  ...
  for (int i = 0; i < S->warriors.len && n < maxn; ++i) {
    ...
    /* base rule: a stack >= MIN_STACK within MAX_HOPS responds.  travel rule
       (no-trickle): ANY enemy that can reach the target while our wave is in
       flight (+ slack) responds, however far and however small its stack. */
    ...
    if (!base_rule) ++g_dbg_travel_reinf_n;
...
#ifdef DEBUG_PATCH
  if (g_dbg_travel_reinf_n > 0)
    fprintf(stderr, "# DBG-TRICKLE t%d resp-sim target=%d eta=%d travel_reinf=%d def_n=%d\n",
            g_current_turn, target, attack_eta, g_dbg_travel_reinf_n, def_n);
#endif
  int building_hp = max_int(0, b->hp);
  int turret = building_turret_power_const(b);
...
static int opponent_is_force_training(const GameState *S, const GameMap *M,
                                      int theirs) {
  /* Hidden bases make visible work capacity only a lower bound.  Do not label
     a visible enemy stack "debt-financed" unless the entire map is visible. */
...
static int issue_army_parity_training(Actions *a, const GameState *S,
  ...
  /* reserve build gold for claim parties already standing on unbuilt
     strongholds - a 500g base outvalues about 4.2 trains (log doctrine_vs_a23:
...
static int collect_enemy_hq_attack_schedule(const GameState *S, const GameMap *M,
  ...
  for (int i = 0; i < S->warriors.len && n < maxn; ++i) {
    ...
    if (eta < 0 && HQ_SIM_DEFENSE_THREAT_RADIUS > 0) {
      /* Enemy destinations are hidden in the new protocol.  A visible enemy
         already this close to our HQ is treated pessimistically as a possible
         immediate attacker; this restores the intended local exact-sim guard
         without inventing a hidden MOVE target. */
...
static int final_hq5_repair_reserve(const GameState *S, const GameMap *M,
  ...
  /* On turn 400, if HQ is already full, there is no later morning on which the
     saved 1000 gold can be converted into HP.  Before turn 400, reserve the
...
/* FINISHER (REPORT 11-6): "we are so far ahead that the spare army should be
   spent on enemy HQ hp".  Deliberately conservative - it must never fire while
   the opponent could counter: a 2:1 ratio in BOTH bodies and hp, a real force,
   an opponent whose income cannot replace it, nothing within
   FINISHER_HOME_SAFE_RADIUS hops of our own HQ, and the HQ fund already settled
   (kill only after our last upgrade lands). */
static int finisher_dominant(const GameState *S, const GameMap *M,
                             const Actions *a, int budget, int turn) {
  /* FINISHER compares global enemy army/base income.  With partial vision,
     visible totals are lower bounds and cannot prove dominance. */
  if (!all_regions_visible_to_me(S, M)) return 0;
...
/* FINISHER: our own HQ level is 5 hp of margin, and an early kill throws it
   away - measured on T4, 7 replays killed the enemy HQ 15-31 turns earlier and
   ended at HQ L4 instead of L5 (margin 25 instead of 30).  So an ASSAULT waits
   while the next level is still reachable with the turns that remain (plus the
   slack needed to walk in and kill afterwards).  Cap training does NOT wait -
   the bodies are what makes the later kill fast. */
...
static int choose_train_count(const GameState *S, const GameMap *M,
  ...
  if (final_hq5_cash_dump) {
    ...
    /* FINISHER: the idle-economy lever - once dominant, gold is worth more as
       bodies walking at the enemy HQ than as unspent cash. */
    cash_dump_train = 1;
#ifdef DEBUG_PATCH
    fprintf(stderr, "# Debug FIN captrain turn %d gold %d\n", turn, budget);
#endif
  }
...
static int thin_opening_train_count(const GameState *S, const GameMap *M,
  ...
  /* For neutral expansion, train at most one body and only when the HQ->target
     ...
     stand on an empty stronghold waiting for 500 gold. */
  int after_gold = budget - TRAIN_COST;
  if (after_gold < 0) return 0;
  int target = choose_trainable_timed_neutral_train_target(S, M, P, a, budget, 1);
  if (target < 0) return 0;
...
static int issue_thin_opening_grab_phase(Actions *a, const GameState *S,
  ...
  /* Pipeline expansion: an earlier worker may already be sitting on or moving
     ...
     would miss its 500-gold build, they fail and no new worker is sent/trained. */
...
static int hard_advantage_phase_active(const GameState *S, const GameMap *M,
                                       int turn) {
  /* Every trigger below compares GLOBAL base/army totals.  Under fog those
     totals are not defined by the current snapshot.  Keep the old conversion
     only when our vision actually covers the whole map. */
  if (!all_regions_visible_to_me(S, M)) return 0;
...
static int collect_stage_candidates(const GameState *S, const GameMap *M,
  ...
  for (int i = 0; i < S->warriors.len && n < maxn; ++i) {
    ...
    if (turn >= 350) keep = min_int(1, remaining); /* late: finish the game */
    (void)keep;
    int already_taken = 0;
...
static int target_is_hq_allowed_now(const GameState *S, const GameMap *M,
  ...
  /* FINISHER (REPORT 11-6): with the enemy bases gone and a 2:1 dominance, the
     HQ finish must not wait for our own HQ5 - that wait is why 76/79 turn-limit
     wins ended with the army standing still. */
  if (FINISHER_HQ_TARGET_RELAX && finisher_attack_allowed(S, M, a, S->gold, turn)) {
#ifdef DEBUG_PATCH
    fprintf(stderr, "# Debug FIN hqtarget turn %d\n", turn);
#endif
    return 1;
  }
...
static int issue_hard_staged_attack(Actions *a, const GameState *S,
  ...
  if (stage_n >= need) {
    /* no-trickle: the wave must also beat the reinforcements that can reach
       the target while it is in flight.  Escalate the wave if the stage holds
       more bodies; refuse the launch when even that is not enough. */
...
static int issue_hard_advantage_conversion(Actions *a, const GameState *S,
  ...
  /* FIX F1 (endgame-nonupgrade-rootcause): a legal+affordable HQ level-up/repair is worth
     more than a turn of staged-attack bookkeeping; issue it before the 'enemy bases first'
     early return below (which fired for 36 turns in 28_233428 and on the last 3 turns of
     30_253354 while 2,400-4,300 gold sat unused at a legal HQ). */
  if (issue_hard_hq_priority_if_possible(a, S, M, P, budget, turn)) {
#ifdef DEBUG_PATCH
    fprintf(stderr, "# Debug F1 hoist fired turn %d\n", turn);
#endif
...
static int issue_endgame_hq_attack(Actions *a, const GameState *S,
  ...
  for (int i = 0; i < S->warriors.len; ++i) {
    ...
    /* Turns 380..390 are synchronized so that issued warriors arrive on the
       enemy HQ in turn 390.  After 390, send all remaining available warriors
...
static int opening_anti_rush_guard_end_turn(const GameMap *M,
                                             const Paths *P) {
#if ENABLE_OPENING_ANTI_RUSH_GUARD
  int h = path_hops_between(P, M->my_hq, M->opp_hq);
  if (h >= INF_HOPS) h = 0;
  return OPENING_ANTI_RUSH_BASE_TURNS + h;
#else
  (void)M; (void)P;
  return -1;
#endif
...
static int enforce_opening_anti_rush_hq_keep(Actions *a,
                                             const GameState *S,
                                             const GameMap *M,
                                             const Paths *P) {
#if ENABLE_OPENING_ANTI_RUSH_GUARD
  if (g_current_turn > opening_anti_rush_guard_end_turn(M, P)) return 0;
  int keep = max_int(OPENING_ANTI_RUSH_HOME_KEEP,
                     planned_work_cap_at(S, M, a, M->my_hq));
  int defenders = planned_hq_defenders_after_actions(S, M, a);
  int cancelled = 0;
  for (int i = a->moves.len - 1; i >= 0 && defenders < keep; --i) {
    const Warrior *w = find_warrior_const(S, a->moves.data[i].id);
    if (w == NULL || w->id.side != M->my_side) continue;
    if (w->region != M->my_hq) continue;
    if (a->moves.data[i].target == M->my_hq) continue;
    remove_move_at(a, i);
    ++defenders;
    ++cancelled;
  }
#ifdef DEBUG_PATCH
  if (cancelled > 0)
    fprintf(stderr, "# DBG-OPEN-GUARD t%d keep=%d cancelled=%d\n",
            g_current_turn, keep, cancelled);
#endif
  return cancelled;
#else
  (void)a; (void)S; (void)M; (void)P;
  return 0;
#endif
}

static int own_hq_wave_units_still_committed(const GameState *S,
                                             const GameMap *M) {
  int n = 0;
  for (int i = 0; i < S->warriors.len; ++i) {
    const Warrior *w = &S->warriors.data[i];
    ...
    if (w->region == M->opp_hq ||
        (w->state == WSTATE_MOVING && w->target == M->opp_hq))
      ++n;
  }
  return n;
}

static void hq_failed_wave_update_resolution(const GameState *S,
                                             const GameMap *M) {
#if ENABLE_HQ_FAILED_WAVE_MEMORY
  if (!g_hq_wave_inflight) return;
  if (own_hq_wave_units_still_committed(S, M) > 0) return;
  const Building *hq = find_building_const(S, M->opp_hq);
  if (hq == NULL || hq->side == M->my_side) {
    g_hq_wave_inflight = 0;
    return;
  }

  int prev = g_hq_wave_inflight_size;
  g_hq_wave_inflight = 0;
  g_hq_wave_inflight_size = 0;
  if (prev < HQ_FAILED_WAVE_MIN_RECORD) return;

  int by_add = prev + HQ_FAILED_WAVE_ADD;
  int by_ratio = (prev * HQ_FAILED_WAVE_RATIO_NUM +
                  HQ_FAILED_WAVE_RATIO_DEN - 1) /
                 HQ_FAILED_WAVE_RATIO_DEN;
  int req = max_int(by_add, by_ratio);
  if (req > HQ_FAILED_WAVE_MAX_REQUIRED) req = HQ_FAILED_WAVE_MAX_REQUIRED;
  if (req > g_hq_wave_required_min) g_hq_wave_required_min = req;
  ++g_hq_wave_failures;
#ifdef DEBUG_PATCH
  fprintf(stderr, "# DBG-HQ-WAVE failed t%d prev=%d next_min=%d failures=%d\n",
          g_current_turn, prev, g_hq_wave_required_min, g_hq_wave_failures);
#endif
#else
  (void)S; (void)M;
#endif
...
static void hq_failed_wave_note_final_actions(const Actions *a,
                                              const GameMap *M) {
#if ENABLE_HQ_FAILED_WAVE_MEMORY
  int sent = 0;
  for (int i = 0; i < a->moves.len; ++i)
    if (a->moves.data[i].target == M->opp_hq) ++sent;
  if (sent < HQ_FAILED_WAVE_MIN_RECORD) return;
  if (!g_hq_wave_inflight) {
    g_hq_wave_inflight = 1;
    g_hq_wave_inflight_size = sent;
    g_hq_wave_launch_turn = g_current_turn;
  } else {
    g_hq_wave_inflight_size += sent;
  }
#ifdef DEBUG_PATCH
  fprintf(stderr, "# DBG-HQ-WAVE launch t%d sent=%d inflight=%d required=%d\n",
          g_current_turn, sent, g_hq_wave_inflight_size, g_hq_wave_required_min);
#endif
#else
  (void)a; (void)M;
#endif
...
static Actions anchor_rush_finish(Actions a, const GameState *S,
  ...
  enforce_opening_anti_rush_hq_keep(&a, S, M, g_stack_guard_paths);
  hq_failed_wave_note_final_actions(&a, M);
  return a;
}
...
static Actions decide(const GameState *S, const GameMap *M, const Paths *P,
  ...
  hq_failed_wave_update_resolution(S, M);
  ...
  /* FIX F2 (endgame-nonupgrade-rootcause, re-integrated): 'train the 600' L2
     starvation.  Issue the HQ L1->L2 upgrade here - after the defense modules took
     their gold, but BEFORE parity / gather-pressure / opening-phase training can
     spend the same 600 one warrior at a time.  GATED on the center-split race being
     over: while center_second_base_opening_active() the 600 is the center-wave
     replacement budget, and spending it desynchronizes the wave, hands the center
     (sole-survivor latch) to the enemy and loses the game (base_v2 regression:
     0.339 T1, first divergence t41 = UPGRADE 0 instead of TRAIN 1). */
...
#ifdef DEBUG_PATCH
        fprintf(stderr, "# Debug F2 fired turn %d\n", turn);
#endif
      }
    }
  }
...
#if ENABLE_OPENING_ANTI_RUSH_GUARD
  if (turn == OPENING_ANTI_RUSH_EXTRA_TRAIN_TURN && a.train_n == 0 &&
      budget >= TRAIN_COST && planned_train_cap(S, M, &a) >= 1) {
    a.train_n = 1;
    budget -= TRAIN_COST;
  }
#endif
  ...
  if (!defense_plan.triggered && center_second_base_opening_active(S, M, turn)) {
    ...
    /* Economy release for a drawn-out center race: past this turn the race
       gate no longer freezes the whole bot.  The center claims above were
       still issued (the center stays contested); hold back the center build
       cost when our contestant already stands on the neutral center so the
       economy below cannot starve tomorrow's build, then fall through to the
       normal economy/upgrade/attack modules. */
#ifdef DEBUG_PATCH
    fprintf(stderr, "center-econ-release turn=%d waves=%d\n", turn,
            g_center_split_waves_sent);
#endif
    {
      ...
      if (crr >= 0 && crr < M->N && find_building_const(S, crr) == NULL) {
        /* Race solvency floor (planner-only).  Always hold the wave retry
           train (120) + move (10) so the cadence never misses a beat.  Once
           the HQ is L2 (the hp-5 duel winner is coming) also hold the 500g
           center build: seed 1 won the duel at t64 but could only build at
           t93, losing the game.  Before HQ2 the 500 is NOT held -- it starves
           the HQ2 purchase itself and re-freezes the whole race. */
  ...
  if (is_endgame_turn(turn)) {
    ...
    if (end_def.triggered) {
      ...
      if (!hard_advantage_phase_active(S, M, turn))
        issue_final_hq5_cash_dump_train(&a, S, M, &budget, turn);
      /* FINISHER (REPORT 11-6): wire the never-called issue_endgame_hq_attack.
         With the map won, nothing near home and the HQ fund settled, every spare
         body (upkeep workers are kept by assign_endgame_keep_workers) is worth
         more as enemy-HQ damage than as an idle garrison: margin IS HQ hp. */
      if (FINISHER_STREAM && finisher_attack_allowed(S, M, &a, budget, turn)) {
        int fin_sent = issue_endgame_hq_attack(&a, S, M, P, &budget, turn);
        (void)fin_sent;
#ifdef DEBUG_PATCH
        fprintf(stderr, "# Debug FIN stream turn %d sent %d\n", turn, fin_sent);
#endif
      }
      issue_final_hq5_anchor_dispatch(&a, S, M, P, &budget, turn);
    }
  ...
  if (!defense_plan.triggered && !enemy_stack_attack_triggered &&
    ...
    if (a.train_n > 0 && !opening_neutral_should_abort_for_pressure(S, M, P)) {
      /* Keep the center-race replacement train: on economy-release turns the
         refund otherwise cancels it and the center race is lost by exactly
         one training tempo. */
      int keep_n =
  ...
  if (!opening_neutral_priority_lock) {
    ...
    /* SAVER (REPORT 11-5): a stack gathering somewhere far away must not drain
       the late HQ fund.  This clearing is why 313/455-style gatherers keep the
       bot one HQ level below the opponent at the turn limit. */
...
#ifdef DEBUG_PATCH
      fprintf(stderr, "# Debug SAVER gather-keep turn %d\n", turn);
#endif
    }
...
: launch only after defense and HQ reservation.
  ...
  /* Opening build debt beats non-defense training.  This catches cases after
     ...
     500-gold construction fund one turn before the build. */
...
/* ===== MoE expert routing (user 2026-07-06): params by map size =====
   legacy expert 0: N<=63 | expert 1: N<=79 | expert 2: N<=93 | expert 3: N>=95
   New-rule maps are N=181..249, so this baseline intentionally retains legacy expert 3
   without retuning its non-turn hyperparameters.
...
static void moe_apply_expert_params(int N) {
  ...
  g_moe_expert = e;
  { static const int t[4] = {3, 3, 3, 3}; P_EVAC_GUARD_MIN_STACK = t[e]; }
  ...
  { static const int t[4] = {85, 85, 85, 85}; P_FORCE_HQ2_SAVE_TURN = t[e]; }
  ...
  { static const int t[4] = {0, 0, 0, 0}; P_PINGPONG_VETO_TURNS = t[e]; }
  ...
  { static const int t[4] = {86, 86, 86, 86}; P_CENTER_SPLIT_ECON_AFTER_TURN = t[e]; }
  { static const int t[4] = {25, 25, 25, 25}; P_CENTER_SPLIT_MAX_FAILED_WAVES = t[e]; }
  { static const int t[4] = {1, 1, 1, 1}; P_ENABLE_OFFENSE_REINF_TRAVEL = t[e]; }
  { static const int t[4] = {1, 1, 1, 1}; P_OFFENSE_REINF_MIN_STACK = t[e]; }
  { static const int t[4] = {0, 0, 0, 0}; P_OFFENSE_REINF_ARRIVAL_SLACK = t[e]; }
  { static const int t[4] = {1, 1, 1, 1}; P_ENABLE_REATTACK_MEMORY = t[e]; }
  { static const int t[4] = {30, 30, 30, 30}; P_REATTACK_FORGET_TURNS = t[e]; }
  /* SAVER / FINISHER (REPORT 11-5 + 11-6) */
  { static const int t[4] = {140, 140, 140, 140}; P_LATE_SAVER_START_TURN = t[e]; }
  { static const int t[4] = {12, 12, 12, 12}; P_LATE_SAVER_LOOKAHEAD_TURNS = t[e]; }
  { static const int t[4] = {2, 2, 2, 2}; P_LATE_SAVER_THREAT_RADIUS = t[e]; }
  { static const int t[4] = {1, 1, 1, 1}; P_LATE_SAVER_THREAT_MIN = t[e]; }
  { static const int t[4] = {1, 1, 1, 1}; P_LATE_SAVER_RELEASE_ON_THREAT = t[e]; }
  { static const int t[4] = {6, 6, 6, 6}; P_LATE_SAVER_GATHER_RADIUS = t[e]; }
  { static const int t[4] = {1, 1, 1, 1}; P_LATE_SAVER_LOCK = t[e]; }
  { static const int t[4] = {150, 150, 150, 150}; P_FINISHER_START_TURN = t[e]; }
  { static const int t[4] = {8, 8, 8, 8}; P_FINISHER_MIN_ARMY = t[e]; }
  { static const int t[4] = {2, 2, 2, 2}; P_FINISHER_ARMY_RATIO_NUM = t[e]; }
  { static const int t[4] = {1, 1, 1, 1}; P_FINISHER_ARMY_RATIO_DEN = t[e]; }
  { static const int t[4] = {150, 150, 150, 150}; P_FINISHER_MAX_ENEMY_INCOME = t[e]; }
  { static const int t[4] = {4, 4, 4, 4}; P_FINISHER_HOME_SAFE_RADIUS = t[e]; }
  { static const int t[4] = {1, 1, 1, 1}; P_FINISHER_CAP_TRAIN = t[e]; }
  { static const int t[4] = {1, 1, 1, 1}; P_FINISHER_HQ_TARGET_RELAX = t[e]; }
  { static const int t[4] = {1, 1, 1, 1}; P_FINISHER_STREAM = t[e]; }
  { static const int t[4] = {1, 1, 1, 1}; P_FINISHER_ATTACK_WAIT_FOR_HQ = t[e]; }
  { static const int t[4] = {15, 15, 15, 15}; P_FINISHER_KILL_SLACK_TURNS = t[e]; }
}

/* 200->400 day horizon adaptation for genuine endgame clocks only.
   Early/midgame thresholds remain tied to travel/economy rather than being
   mechanically doubled.  Local combat horizons/lookaheads are unchanged. */
static void apply_new_rule_phase_turn_scaling(void) {
  /* Only true end-of-game clocks scale with the 200 -> 400 turn limit.
     Opening, HQ-progression, rally and center-race thresholds are governed by
     travel/economy and should NOT be doubled: doing so delayed HQ2, delayed
     scouting, and could keep a symmetric center duel frozen for 170+ turns. */
  P_ENDGAME_SYNC_START_TURN *= 2;
  P_ENDGAME_SYNC_ARRIVAL_TURN *= 2;
  P_LATE_TIEBREAK_START_TURN *= 2;
  P_LATE_SAVER_START_TURN *= 2;
  P_FINISHER_START_TURN *= 2;
  P_CASH_DUMP_TRAIN_START_TURN *= 2;
  P_FINAL_HQ5_CASH_DUMP_START_TURN *= 2;
  P_FINAL_HQ5_HARD_DUMP_START_TURN *= 2;
}


/* Rule-compatibility overrides for the new economy/fog map.  These repair old
   assumptions rather than searching a new hyperparameter optimum. */
static void apply_new_rule_structural_params(void) {
  /* Generic training must not consume the money needed for the next legal
     scout/claim: 500 build + 10 move. */
  P_BUILD_RESERVE = BASE_LEVELS[1].cost + MOVE_COST;
  /* The old large-map expert kept work_cap+3 at every source.  With 500g bases
     this leaves no movable scout after thin opening. */
  if (P_NORMAL_EXTRA_GARRISON > 1) P_NORMAL_EXTRA_GARRISON = 1;
  /* Legacy expert-3 disabled these against specific full-information opponents.
     In fog, newly revealed enemy infrastructure must remain attackable. */
  P_ENABLE_ENEMY_BASE_CAPTURE = 1;
  P_ENABLE_ENEMY_HQ_ATTACK = 1;
  /* Legacy large-map expert could read exact enemy MOVE targets and therefore
     used radius 0 for base crisis detection.  Targets are hidden now, so a
     visible enemy one hop from an owned base must enter the local crisis sim. */
  P_BASE_SIM_DEFENSE_THREAT_RADIUS = 1;
  /* The legacy large-map expert can hard-gate normal economy until turn 86 and
     retry 25 one-body center waves.  Under the larger fog map this can turn a
     symmetric center contest into a long self-imposed freeze.  Keep the center
     opening, but release normal economy promptly and bound dedicated retries. */
  if (P_CENTER_SPLIT_ECON_AFTER_TURN > 30) P_CENTER_SPLIT_ECON_AFTER_TURN = 30;
  if (P_CENTER_SPLIT_MAX_FAILED_WAVES > 4) P_CENTER_SPLIT_MAX_FAILED_WAVES = 4;
}
...
int main(void) {
  ...
  moe_apply_expert_params(M.N);   /* new-rule N=181..249 selects legacy large-map expert */
  apply_new_rule_structural_params(); /* repair new economy/fog assumptions */
  apply_new_rule_phase_turn_scaling(); /* scale only genuine endgame clocks */
