/* INHYUK V2 (2026-08-29): behavior-preserving refactor of inhyuk_v1.cpp. */
...
#define OPENING_NEUTRAL_FIRST_MAX_TURN 123
#endif
#ifndef OPENING_NEUTRAL_FIRST_TARGET_PERCENT
#define OPENING_NEUTRAL_FIRST_TARGET_PERCENT 82
#endif
...
#define OPENING_NEUTRAL_FIRST_MAX_TARGETS 19
#endif
...
#define THIN_OPENING_MAX_TURN 46
#endif
...
#endif
#ifndef ENABLE_CONFIDENCE_HQ_RESERVE
#define ENABLE_CONFIDENCE_HQ_RESERVE 1
#endif
#ifndef POTENTIAL_RUSH_MIN_CLUSTER
#define POTENTIAL_RUSH_MIN_CLUSTER 8
#endif
#ifndef POTENTIAL_RUSH_CLUSTER_RADIUS
#define POTENTIAL_RUSH_CLUSTER_RADIUS 1
#endif
#ifndef POTENTIAL_RUSH_MAX_HOPS
#define POTENTIAL_RUSH_MAX_HOPS 10
#endif
#ifndef POTENTIAL_RUSH_MEDIUM_HOPS
#define POTENTIAL_RUSH_MEDIUM_HOPS 6
#endif
#ifndef POTENTIAL_RUSH_HIGH_HOPS
#define POTENTIAL_RUSH_HIGH_HOPS 3
#endif
#ifndef POTENTIAL_RUSH_MEDIUM_INBOUND_PERCENT
#define POTENTIAL_RUSH_MEDIUM_INBOUND_PERCENT 40
#endif
#ifndef POTENTIAL_RUSH_HIGH_INBOUND_PERCENT
#define POTENTIAL_RUSH_HIGH_INBOUND_PERCENT 70
#endif
#ifndef POTENTIAL_RUSH_BASE_ATTACK_RADIUS
#define POTENTIAL_RUSH_BASE_ATTACK_RADIUS 1
#endif
#ifndef POTENTIAL_RUSH_RESERVE_MARGIN
#define POTENTIAL_RUSH_RESERVE_MARGIN 2
#endif
#ifndef POTENTIAL_RUSH_LOW_RESERVE_PERCENT
#define POTENTIAL_RUSH_LOW_RESERVE_PERCENT 25
#endif
#ifndef POTENTIAL_RUSH_MEDIUM_RESERVE_PERCENT
#define POTENTIAL_RUSH_MEDIUM_RESERVE_PERCENT 50
#endif
#ifndef POTENTIAL_RUSH_MEMORY_TURNS
#define POTENTIAL_RUSH_MEMORY_TURNS 4
#endif
#ifndef POTENTIAL_RUSH_RESERVE_DECAY_PER_TURN
#define POTENTIAL_RUSH_RESERVE_DECAY_PER_TURN 2
#endif
#ifndef POTENTIAL_RUSH_MEDIUM_RECALL_MAX_HOPS
#define POTENTIAL_RUSH_MEDIUM_RECALL_MAX_HOPS 6
#endif
#ifndef POTENTIAL_RUSH_HIGH_RECALL_MAX_HOPS
#define POTENTIAL_RUSH_HIGH_RECALL_MAX_HOPS 10
#endif
#ifndef POTENTIAL_RUSH_RECALL_ETA_SLACK
#define POTENTIAL_RUSH_RECALL_ETA_SLACK 1
#endif
...
#define ANCHOR_ROUTE_START_TURN 133
#endif
...
#define ANCHOR_ROUTE_MIN_OWNED_BASES_TO_ATTACK 13
#endif
...
#define FINISHER_START_TURN 300
#endif
#ifndef FINISHER_MIN_ARMY
#define FINISHER_MIN_ARMY 8
#endif
...
#endif
#ifndef FORCE_HQ2_MAX_SAVE_TURNS
#define FORCE_HQ2_MAX_SAVE_TURNS 20
#endif
...
static int g_confidence_hq_reserve_units = 0;
static int g_confidence_hq_reserve_tier = 0;
static int g_confidence_hq_reserve_until_turn = -1;
...
static int issue_forced_hq2_save_or_upgrade(Actions *a, const GameState *S,
  ...
  int has_hq_worker = count_warriors_at(S, M->my_side, M->my_hq) > 0;
  if (!has_hq_worker) {
    if (force_hq2_upgrade_worker(a, S, M, P, budget)) return 1;
    return 0;
  }
  ...
  const Building *hq = find_building_const(S, M->my_hq);
  if (hq == NULL || hq->level != 1) return 0;
  int cost = building_upgrade_cost(hq);
  if (*budget >= cost) return 0;
  ...
  if (net <= 0) return 0;
  int gap = cost - *budget;
  int save_turns = (gap + net - 1) / net;
  if (save_turns > FORCE_HQ2_MAX_SAVE_TURNS) return 0;
  return 1;
}
...
typedef struct {
  ...
  int base_attack;
  int reserve_target;
} PotentialRushAssessment;

static int enemy_pressures_owned_base(const GameState *S, const GameMap *M,
                                      const Paths *P, const Warrior *w) {
  int prev_r = previous_region_of(w->id);
  int prev_turn = previous_seen_turn_of(w->id);
  for (int bi = 0; bi < S->buildings.len; ++bi) {
    ...
    int cur = path_hops_between(P, w->region, b->region);
    if (cur > POTENTIAL_RUSH_BASE_ATTACK_RADIUS) continue;
    if (cur == 0) return 1;
    if (prev_r < 0 || g_current_turn - prev_turn > DEFENSE_THREAT_MEMORY_TURNS)
      continue;
    int prev = path_hops_between(P, prev_r, b->region);
    if (prev < INF_HOPS && cur < prev) return 1;
  }
  return 0;
}

static int potential_rush_member_inbound(const GameMap *M, const Paths *P,
                                         const Warrior *w) {
  int prev_r = previous_region_of(w->id);
  int prev_turn = previous_seen_turn_of(w->id);
  if (prev_r < 0 || g_current_turn - prev_turn > DEFENSE_THREAT_MEMORY_TURNS)
    return 0;
  int cur_h = path_hops_between(P, w->region, M->my_hq);
  int prev_h = path_hops_between(P, prev_r, M->my_hq);
  return cur_h < INF_HOPS && prev_h < INF_HOPS && cur_h < prev_h;
}

static int confidence_reserve_full_target(const GameState *S,
                                          const GameMap *M,
                                          const Actions *a,
                                          int enemy_count, int enemy_hp) {
  const Building *hq = find_building_const(S, M->my_hq);
  int my_hp = HQ_LEVELS[1].warrior_hp;
  if (hq != NULL && hq->side == M->my_side && hq->type == BTYPE_HQ &&
      hq->level >= 1 && hq->level <= HQ_MAX_LEVEL)
    my_hp = HQ_LEVELS[hq->level].warrior_hp;
  int hp_need = (enemy_hp + my_hp - 1) / my_hp;
  int required = max_int(enemy_count, hp_need) + POTENTIAL_RUSH_RESERVE_MARGIN;
#if HOME_DEFENSE_KEEP_WORKERS
  required = max_int(required, planned_work_cap_at(S, M, a, M->my_hq));
#endif
  return required;
}

static PotentialRushAssessment assess_potential_hq_rush(
    const GameState *S, const GameMap *M, const Paths *P, const Actions *a) {
  PotentialRushAssessment best;
  memset(&best, 0, sizeof(best));
  best.min_hq_hops = INF_HOPS;
#if ENABLE_CONFIDENCE_HQ_RESERVE
  Side opp = opposite(M->my_side);
  for (int si = 0; si < S->warriors.len; ++si) {
    const Warrior *seed = &S->warriors.data[si];
    if (seed->id.side != opp) continue;
    int seed_h = path_hops_between(P, seed->region, M->my_hq);
    if (seed_h > POTENTIAL_RUSH_MAX_HOPS) continue;

    int cnt = 0, hp = 0, inbound = 0, base_pressure = 0;
    int min_h = INF_HOPS;
    for (int j = 0; j < S->warriors.len; ++j) {
      const Warrior *w = &S->warriors.data[j];
      if (w->id.side != opp) continue;
      if (path_hops_between(P, seed->region, w->region) >
          POTENTIAL_RUSH_CLUSTER_RADIUS)
        continue;
      int h = path_hops_between(P, w->region, M->my_hq);
      if (h > POTENTIAL_RUSH_MAX_HOPS) continue;
      ++cnt;
      hp += max_int(1, w->hp);
      if (h < min_h) min_h = h;
      if (potential_rush_member_inbound(M, P, w)) ++inbound;
      if (enemy_pressures_owned_base(S, M, P, w)) ++base_pressure;
    }
    if (cnt < POTENTIAL_RUSH_MIN_CLUSTER || min_h >= INF_HOPS) continue;

    int inbound_pct = (100 * inbound) / cnt;
    int tier = 1;
    if (min_h <= POTENTIAL_RUSH_MEDIUM_HOPS ||
        inbound_pct >= POTENTIAL_RUSH_MEDIUM_INBOUND_PERCENT)
      tier = 2;
    if (min_h <= POTENTIAL_RUSH_HIGH_HOPS ||
        inbound_pct >= POTENTIAL_RUSH_HIGH_INBOUND_PERCENT)
      tier = 3;

    int base_attack = (base_pressure * 2 >= cnt);
    if (base_attack && min_h > POTENTIAL_RUSH_HIGH_HOPS && tier > 1)
      --tier;

    int full = confidence_reserve_full_target(S, M, a, cnt, hp);
    int target = full;
    if (tier == 1)
      target = (full * POTENTIAL_RUSH_LOW_RESERVE_PERCENT + 99) / 100;
    else if (tier == 2)
      target = (full * POTENTIAL_RUSH_MEDIUM_RESERVE_PERCENT + 99) / 100;
#if HOME_DEFENSE_KEEP_WORKERS
    target = max_int(target, planned_work_cap_at(S, M, a, M->my_hq));
#endif

    long long score = (long long)tier * 100000000LL +
                      (long long)cnt * 100000LL - min_h;
    long long best_score = (long long)best.tier * 100000000LL +
                           (long long)best.count * 100000LL - best.min_hq_hops;
    if (best.tier == 0 || score > best_score) {
      best.tier = tier;
      best.count = cnt;
      best.hp = hp;
      best.min_hq_hops = min_h;
      best.inbound_count = inbound;
      best.base_attack = base_attack;
      best.reserve_target = target;
    }
  }
#else
  (void)S; (void)M; (void)P; (void)a;
#endif
  return best;
}

static int confidence_hq_reserve_update(const GameState *S,
                                        const GameMap *M,
                                        const Actions *a,
                                        const PotentialRushAssessment *r,
                                        int turn) {
#if ENABLE_CONFIDENCE_HQ_RESERVE
  int floor_keep = 0;
#if HOME_DEFENSE_KEEP_WORKERS
  floor_keep = planned_work_cap_at(S, M, a, M->my_hq);
#endif
  if (r->tier > 0) {
    int target = r->reserve_target;
    if (target >= g_confidence_hq_reserve_units)
      g_confidence_hq_reserve_units = target;
    else
      g_confidence_hq_reserve_units = max_int(
          target, g_confidence_hq_reserve_units - POTENTIAL_RUSH_RESERVE_DECAY_PER_TURN);
    g_confidence_hq_reserve_tier = r->tier;
    g_confidence_hq_reserve_until_turn = turn + POTENTIAL_RUSH_MEMORY_TURNS;
  } else if (turn <= g_confidence_hq_reserve_until_turn) {
    g_confidence_hq_reserve_units = max_int(
        floor_keep, g_confidence_hq_reserve_units - POTENTIAL_RUSH_RESERVE_DECAY_PER_TURN);
    if (g_confidence_hq_reserve_units <= floor_keep)
      g_confidence_hq_reserve_tier = 0;
  } else {
    g_confidence_hq_reserve_units = 0;
    g_confidence_hq_reserve_tier = 0;
  }
  return g_confidence_hq_reserve_units;
#else
  (void)S; (void)M; (void)a; (void)r; (void)turn;
  return 0;
#endif
}
...
static int planned_hq_defender_count(const GameState *S,
                                      const GameMap *M,
                                      const Actions *a) {
  int cnt = a->train_n;
  for (int i = 0; i < S->warriors.len; ++i) {
    ...
    int planned_target = -1;
    for (int mi = 0; mi < a->moves.len; ++mi) {
      if (wid_eq(a->moves.data[mi].id, w->id)) {
        planned_target = a->moves.data[mi].target;
        break;
      }
    }
    if (planned_target >= 0) {
      if (planned_target == M->my_hq) ++cnt;
      continue;
    }
    if (w->region == M->my_hq ||
        (w->state == WSTATE_MOVING && w->target == M->my_hq))
...
static int issue_confidence_hq_reserve_fill(
    Actions *a, const GameState *S, const GameMap *M, const Paths *P,
    int *budget, const PotentialRushAssessment *r) {
#if ENABLE_CONFIDENCE_HQ_RESERVE
  if (r->tier < 2 || r->reserve_target <= 0) return 0;
  int need = r->reserve_target - planned_hq_defender_count(S, M, a);
  if (need <= 0) return 0;

  int max_recall = (r->tier >= 3) ? POTENTIAL_RUSH_HIGH_RECALL_MAX_HOPS
                                  : POTENTIAL_RUSH_MEDIUM_RECALL_MAX_HOPS;
  if (r->min_hq_hops < INF_HOPS)
    max_recall = min_int(max_recall,
                         r->min_hq_hops + POTENTIAL_RUSH_RECALL_ETA_SLACK);

  DirectHqDefenderCandidate *cands = (DirectHqDefenderCandidate *)malloc(
      (size_t)S->warriors.len * sizeof(*cands));
  int cand_n = 0, issued = 0;
  if (cands != NULL)
    cand_n = direct_hq_collect_recall_candidates(S, M, P, a, cands,
                                                 S->warriors.len);
  for (int i = 0; i < cand_n && need > 0; ++i) {
    if (cands[i].eta > max_recall) continue;
  ...
  if (r->tier >= 3) {
    need = r->reserve_target - planned_hq_defender_count(S, M, a);
    if (need > 0 && a->train_n == 0) {
      int cap = planned_train_cap(S, M, a);
  ...
  return issued;
#else
  (void)a; (void)S; (void)M; (void)P; (void)budget; (void)r;
  return 0;
#endif
}
...
static Actions decide(const GameState *S, const GameMap *M, const Paths *P,
  ...
  PotentialRushAssessment potential_rush =
      assess_potential_hq_rush(S, M, P, &a);
  g_home_defense_forced_hq_keep = max_int(
      g_home_defense_forced_hq_keep,
      confidence_hq_reserve_update(S, M, &a, &potential_rush, turn));
  issue_confidence_hq_reserve_fill(&a, S, M, P, &budget, &potential_rush);
